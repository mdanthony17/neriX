...
NerixAnalysisManager::EndOfEvent(const G4Event *pEvent)
{
	...
	if(pHCofThisEvent)
	{
		...
		//printf("LScint Col (in AnalysisManager): %d\n", m_iLiquidScintillatorHitsCollectionID);
		if(m_iLiquidScintillatorHitsCollectionID != -1)
		{
...
//        if(iNbPmtHits > 0)
        if(fTotalEnergyDeposited > 0. || fTotalEnergyDepositedLiquidScintillator > 0.)
			m_pTree->Fill();
        //keeps anything that deposits energy in liquid xenon or liquid scintillator
...
NerixAnalysisManager::Step(const G4Step *pStep)
{
	...
	if(pTrack->GetDefinition()->GetParticleName() == "neutron")
	{
		if(pStep->GetPostStepPoint()->GetPhysicalVolume())
		{
			...
			//printf("%s\n\n\n", (char *)hPreVolumeName);
			//std::cout << hProcessName << std::endl;
