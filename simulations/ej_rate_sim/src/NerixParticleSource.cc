...
NerixParticleSource::NerixParticleSource()
{
	...
	m_hParticleMomentumDirection = G4ParticleMomentum(0., -1., 0.);
...
NerixParticleSource::GeneratePointSource()
{
    ...
    G4ThreeVector m_sourcePosition(0,position_source_y*cm,-1.5*cm); //just outside minitron
	if(m_hSourcePosType == "Point")
        m_hParticlePosition = m_sourcePosition;
		//m_hParticlePosition = m_hCenterCoords; // change position
