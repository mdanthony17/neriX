...
void
PeakFinder::findS1PeaksOnly(Waveform const *pWaveform, std::map<int, Waveform *> const *pWaveforms, bool bOrderByCoincidences)
{
	...
	int iMax = m_iNbSamples;
	int iLastS1Boundary = 0;

	for (int iSample = 0; iSample < iMax; iSample++)
	{
		// if we go above threshold, check if this could be an S1 peak
		if((*m_pWaveform)[iSample] > m_fS1SignalThreshold)
		{
