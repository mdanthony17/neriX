...
AnalysisManager::findPeaks()
{
	...
	// find S1 peaks
	m_pPeakFinder->findS1PeaksOnly(m_pEventData->m_pS1PeakFindingTotalWaveform, m_pEventData->m_pWaveforms, true);
...
AnalysisManager::getSignalsRaw()
{
	...
	*/
	...
	int chanTrig = 3;
	if(pWaveforms->count(chanTrig))
	{
		const Waveform *pTriggerWaveform = (*pWaveforms)[m_iFirstTriggerEfficiencyChannel];
		
		float triggerThreshold = 0.05; // fill in threshold using cout below
		int iNbSamples = (*pWaveforms)[chanTrig]->getNbSamples();
		int startSample = -1;
		int endSample = -1;
