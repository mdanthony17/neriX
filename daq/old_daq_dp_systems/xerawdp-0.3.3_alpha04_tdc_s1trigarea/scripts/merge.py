...
def merge(xml_config, dataset, first, last):
	...
	# change max tree size from 10GB to 50GB on Aug 17, 2015 by MarcW
	if do_chain0:
		chain0.SetMaxTreeSize(50000000000)
	if do_chain1:
		chain1.SetMaxTreeSize(50000000000)
	if do_chain2:
		chain2.SetMaxTreeSize(50000000000)
	if do_chain3:
		chain3.SetMaxTreeSize(50000000000)
	...
	for i in range(first, last+1):
		...
		if not file or file.IsZombie() or (do_chain0 and not file.Get('T0')) or (do_chain1 and not file.Get('T1')) or (do_chain2 and not file.Get('T2')) or (do_chain3 and not file.Get('T3')):
...
if __name__ == '__main__':
	...
	ret = merge(xml_config, dataset, first, last)
