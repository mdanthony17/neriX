...
public:
	...
	int getTrigLeftEdge(int iTrigPeak) const { return (*m_pTrigLeftEdge)[iTrigPeak]; }
	float getTrigArea(int iTrigPeak) const { return (*m_pTrigArea)[iTrigPeak]; }
#endif
...
#ifdef ENABLE_AREATRIG
	std::vector<float> *m_pS1sBottomTrig;
#endif
