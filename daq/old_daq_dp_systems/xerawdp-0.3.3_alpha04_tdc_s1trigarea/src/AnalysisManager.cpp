...
#ifdef ENABLE_AREATRIG
	m_pLevel2Branches->push_back(Branch("S1sBottomTrig", "vector<float>", &m_pEventData->m_pS1sBottomTrig));
#endif
...
#ifdef ENABLE_AREATRIG
		//integrate waveforms for each pmt around each identified trigger signal; added 08-04-2015, Marc W
		//the added variable(s) can be used to retrieve coincident pmt signals which are too small to be identified by the S1 peak finder
		
		int iNbTrigPeaks = m_pEventData->m_pTrigLeftEdge->size(); //get number of trigger peaks identified before
		int	iIntegrationWindowHalfSize = 50; //Set half width of integration window (number of samples) around the TrigLeft position
		//std::cout << std::endl << "\nCalculate waveform area in trigger window; NbTrigPeak = " << iNbTrigPeaks << std::endl;
		
		//loop over each identified trigger peak in channel 19 (see code above how trigger peaks are identified) 
		for (int iTrigPeak = 0; iTrigPeak < iNbTrigPeaks; iTrigPeak++)
		{
			
			int iPmt = m_iFirstBottomPmt; //compute waveform area around trigger window only for bottom PMT
			
			float fAreaRawTriggerWindow = 0.;
			float fS1TriggerWindow = 0.;
			
			if (pWaveforms->count(iPmt-1)) //check if waveform exists for given pmt
			{				
				const Waveform *pRawWaveform = (*pWaveforms)[iPmt-1];
			
				//determine waveform integral and convert into collected charge (e-)
				float fExternalGain = m_pExternalGainsTable->find(iPmt)->second;
				float fToElectronConversionFactor = 1./(m_fInputImpedance*fExternalGain*m_fElectricCharge);					
				fAreaRawTriggerWindow = fToElectronConversionFactor * pRawWaveform->integrate(m_pEventData->getTrigLeftEdge(iTrigPeak) - iIntegrationWindowHalfSize,m_pEventData->getTrigLeftEdge(iTrigPeak) + iIntegrationWindowHalfSize);
				
				//convert raw waveform integral into S1 peak area using pmt gain from xml file
				float fGain = (m_pPmtCalibrationTable->find(iPmt)->second).find("gain")->second;
				fS1TriggerWindow = (fGain != 0.)?(((fAreaRawTriggerWindow > 0.)?(fAreaRawTriggerWindow):(0.))/fGain):(0.);
			}
			
			//store results for bottom pmt for each trigger peak
			m_pEventData->m_pS1sBottomTrig->push_back(fS1TriggerWindow);
		}		
#endif
