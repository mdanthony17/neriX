...
FannAlgorithm::reconstructPosition(const std::vector<float> &hS2)
{
	...
	std::vector<float> sortedS2;
	for (int i = 0; i < 16; i++)
	{
		sortedS2.push_back(m_pfS2[i]);
	}
	std::sort(sortedS2.begin(), sortedS2.end());
	
	if ( ( (sortedS2[16-7]+sortedS2[16-8]+sortedS2[16-9]) / (sortedS2[16-1]+sortedS2[16-2]) ) < 0.6)
	{
	...
	float fX = pfOutput[0]*21.;
	float fY = pfOutput[1]*21.;
