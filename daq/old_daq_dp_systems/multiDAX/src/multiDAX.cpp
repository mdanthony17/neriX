...
//    printf("adc.BufferSize: %d", adc.BufferSize);
	...
	if (WriteToFile>0 && FileEvtCounter==-1) {
	  if (WriteToFile!=3) {
 	    ...
 	    if (control_openxdio(&xfile,&xatt, OutFileName,FileCounter,WriteToFile,
							    adc,logfile,EventsPerFile,t)<0) goto exit_prog;	
	  }
    	  FileCounter++;
	  FileEvtCounter=0;
	}    	
...
//          printf("\n");
		...
		if (firstEvent) { // initialize (old values)
		...
		for (int i=0; i<adc.NumberOfADCs; i++) { // loop over all boards -- check TTT
		  ...
		  if (abs(newTTT-adc.ttt0[i]-deltaT)>max) max=abs(newTTT-adc.ttt0[i]-deltaT); // get max deviation
