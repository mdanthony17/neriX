...
typedef std::vector<std::string> SingleIntegralTableHeader;
typedef std::map<std::string,float> SingleIntegralTableRow;
typedef std::map<int, SingleIntegralTableRow> SingleIntegralTable;
...
public:
...
#ifdef ENABLE_TRIG
	// trigger monitoring
	const std::string &getTrigChannels() const { return m_hTrigChannels; }

	int getFirstTrigChannel() const { return m_iFirstTrigChannel; }
	int getLastTrigChannel() const { return m_iLastTrigChannel; }

	float getTrigSignalThresholdHeight() const { return m_fTrigSignalThresholdHeight; }
	float getTrigSignalThresholdArea() const { return m_fTrigSignalThresholdArea; }
#endif
	...
	int getS1Delay() const { return m_iS1Delay; }
	int getS1DelayTime() const { return m_iS1DelayTime; }
	...
	SingleIntegralTable const *getSingleIntegralTable() const { return &m_hSingleIntegralTable; }
...
private:
	...
	bool readS1LiqSciIntegralSettings(const xmlpp::Node* pRootNode);
#endif
#ifdef ENABLE_NAI
	bool readNaiSettings(const xmlpp::Node* pRootNode);
#endif
#ifdef ENABLE_TRIG
	bool readTrigSettings(const xmlpp::Node* pRootNode);
#endif
#ifdef ENABLE_HPGE
	...
	void parseSingleIntegralWindows(const std::string &hValues);
...
private:
	...
	// the following are for s1 integral
	...
	std::string m_hSingleIntegralWindowValues;
	SingleIntegralTableHeader m_hSingleIntegralTableHeader;
	SingleIntegralTable m_hSingleIntegralTable;
