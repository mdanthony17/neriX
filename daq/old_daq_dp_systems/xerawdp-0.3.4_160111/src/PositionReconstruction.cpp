...
{
	float fS2TotTop = accumulate(hS2.begin(), hS2.begin()+m_iNbTopPmts, 0.);
	double s2_mean = fS2TotTop / hS2.size();
	double s2_sq_sum = std::inner_product(hS2.begin(), hS2.begin()+m_iNbTopPmts, hS2.begin(), 0.0);
	double s2_stdev = std::sqrt(s2_sq_sum / hS2.size() - s2_mean * s2_mean);
	...
	if(m_pRelativeEfficienciesTable && m_pRelativeEfficienciesTable->size())
	{
		...
		*/
		...
		{
			float fRelativeEfficiency = m_pRelativeEfficienciesTable->find(iPmtIndex+1)->second;
			m_pfS2[iPmtIndex] = (hS2[iPmtIndex] - s2_mean) / s2_stdev;
		}
	}
	else
	{
