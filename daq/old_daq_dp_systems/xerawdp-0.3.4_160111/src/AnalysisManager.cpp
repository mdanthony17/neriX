	...
	if(pWaveforms->count(m_iFirstTrigChannel))
	{
		...
		for (int iSample = 0; iSample < iNbSamples; iSample++)
		{
			...
			if(fTriggerSignalHeight > m_fTrigSignalThresholdHeight)
			{
				...
				{
					endSample = iSample;
					if(endSample==iNbSamples-1)
					{
						m_pEventData->m_pTrigLength->push_back(endSample-startSample+2);
					}
				}
			}
			else if(fTriggerSignalHeight < m_fTrigSignalThresholdHeight && endSample > 0)
			{
