...
float Waveform::median(int iStart, int iEnd) const
{
	const int iWindow = iEnd - iStart + 1;
	int aSamples[iWindow];

	for(int i = 0; i < iWindow; i++)
	{
		//std::cout << "sample " << i << ": " << (*this)[iStart+i] << "\n\n";
		aSamples[i] = (*this)[iStart+i];
	}

	std::sort(aSamples, aSamples+iWindow);
	//std::cout << "median: " << vSamples[(int)(vSamples.size()/2)] << "\n\n";
	
	return aSamples[(int)(iWindow/2)];
}
