...
//   Implementation of the CCCUSBCommand class.
...
*/
CCCUSBCommand::CCCUSBCommand(CTCLInterpreter& interp,
...
*/
int
CCCUSBCommand::operator()(CTCLInterpreter& interp,
...
*/
int
CCCUSBCommand::create(CTCLInterpreter& interp,
		       std::vector<CTCLObject>& objv)
{
  ...
  CCCUSBModule* pAdc   = new CCCUSBModule;
...
*/
int
CCCUSBCommand::config(CTCLInterpreter& interp,
		      std::vector<CTCLObject>& objv)
{
  ...
  CReadoutModule* pModule  = m_Config.findAdc(name);
  if(!pModule) {
    Usage("CC-USB module does not exist", objv);
    return TCL_ERROR;
  }
  // and configure:
...
*/
int
CCCUSBCommand::cget(CTCLInterpreter& interp,
		    std::vector<CTCLObject>& objv)
{
  if (objv.size() != 3) {
...
*/
CConfiguration*
CCCUSBCommand::getConfiguration()
{
  return &m_Config;
}
...
*/
void
CCCUSBCommand::Usage(std::string msg, std::vector<CTCLObject> objv)
{
  string result("ERROR: ");
  result += msg;
  result += "\n";
  ...
  result += "\n";
  result += "Usage\n";
  result += "    ccusb create name  n\n";
  result += "    ccusb config name config-params...\n";
  result += "    ccusb cget name";
...
*/
int
CCCUSBCommand:: configure(CTCLInterpreter&         interp,
...
*/
string
CCCUSBCommand::configMessage(std::string base,
			    std::string key,
			    std::string value,
