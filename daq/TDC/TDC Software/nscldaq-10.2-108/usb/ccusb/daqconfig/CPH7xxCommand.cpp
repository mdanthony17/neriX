...
//   Implementation of the CPH7xxCommand class.
...
*/
CPH7xxCommand::CPH7xxCommand(CTCLInterpreter& interp,
...
*/
int
CPH7xxCommand::operator()(CTCLInterpreter& interp,
...
*/
int
CPH7xxCommand:: create(CTCLInterpreter& interp,
		       std::vector<CTCLObject>& objv)
{
...
*/
int
CPH7xxCommand::config(CTCLInterpreter& interp,
		      std::vector<CTCLObject>& objv)
{
  ...
  CReadoutModule* pModule  = m_Config.findAdc(name);
  if(!pModule) {
    Usage("Ph7xxx module does not exist", objv);
    return TCL_ERROR;
  }
  // and configure:
...
*/
int
CPH7xxCommand::cget(CTCLInterpreter& interp,
		    std::vector<CTCLObject>& objv)
{
  if (objv.size() != 3) {
...
*/
CConfiguration*
CPH7xxCommand::getConfiguration()
{
  return &m_Config;
}
...
*/
void
CPH7xxCommand::Usage(std::string msg, std::vector<CTCLObject> objv)
{
  string result("ERROR: ");
  result += msg;
  result += "\n";
  ...
  result += "\n";
  result += "Usage\n";
  result += "    ph7xxx create name base-address\n";
  result += "    ph7xxx config name config-params...\n";
  result += "    ph7xxx cget name";
...
*/
int
CPH7xxCommand:: configure(CTCLInterpreter&         interp,
...
*/
string
CPH7xxCommand::configMessage(std::string base,
			    std::string key,
			    std::string value,
