...
//   Implementation of the CPH7132Command class.
...
*/
CPH7132Command::CPH7132Command(CTCLInterpreter& interp,
...
*/
int
CPH7132Command::operator()(CTCLInterpreter& interp,
...
*/
int
CPH7132Command::create(CTCLInterpreter& interp,
		       std::vector<CTCLObject>& objv)
{
...
*/
int
CPH7132Command::config(CTCLInterpreter& interp,
		      std::vector<CTCLObject>& objv)
{
  ...
  CReadoutModule* pModule  = m_Config.findAdc(name);
  if(!pModule) {
    Usage("ph7132 module does not exist", objv);
    return TCL_ERROR;
  }
  // and configure:
...
*/
int
CPH7132Command::cget(CTCLInterpreter& interp,
		    std::vector<CTCLObject>& objv)
{
  if (objv.size() != 3) {
...
*/
CConfiguration*
CPH7132Command::getConfiguration()
{
  return &m_Config;
}
...
*/
int
CPH7132Command:: configure(CTCLInterpreter&         interp,
...
*/
string
CPH7132Command::configMessage(std::string base,
			    std::string key,
			    std::string value,
