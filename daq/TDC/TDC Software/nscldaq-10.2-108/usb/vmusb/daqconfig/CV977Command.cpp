...
*/
CV977Command::CV977Command(CTCLInterpreter& interp,
...
int
CV977Command::operator()(CTCLInterpreter& interp,
...
*/
int
CV977Command::create(CTCLInterpreter& interp, vector<CTCLObject>& objv)
{
  ...
  // OK so it's not actually an adc but a bit register...

  CV977  * pAdc   = new CV977;
...
*/
int
CV977Command::config(CTCLInterpreter& interp,
		     vector<CTCLObject>& objv)
{
  if (objv.size() < 4) {
...
*/
int
CV977Command::cget(CTCLInterpreter& interp,
		   vector<CTCLObject>& objv)
{
  if (objv.size() != 3) {
...
*/
void
CV977Command::Usage(string msg, std::vector<CTCLObject>& objv)
{
  string result("ERROR: ");
  result += msg;
  result += "\n";
...
*/
int
CV977Command::configure(CTCLInterpreter& interp,
