...
/*!
  Construct the tdc1x90 command and register it.
  ...
  \param commandName - Name of the command to register, defaults to "tdc1x90"
*/

CV1x90Command::CV1x90Command(CTCLInterpreter& interp,
...
*/
int
CV1x90Command::operator()(CTCLInterpreter&    interp,
...
*/
int
CV1x90Command::create(CTCLInterpreter& interp, vector<CTCLObject>& objv)
{
  ...
  if (status != TCL_OK) {
    ...
    m_Config.addAdc(pModule);	// Yeah I know it's a tdc, but this is a legacy name.
...
*/
int
CV1x90Command::config(CTCLInterpreter& interp,
		     vector<CTCLObject>& objv)
{
  if (objv.size() < 4) {
...
*/
int
CV1x90Command::cget(CTCLInterpreter& interp,
		   vector<CTCLObject>& objv)
{
  if (objv.size() != 3) {
...
*/
void
CV1x90Command::Usage(string msg, std::vector<CTCLObject>& objv)
{
  string result("ERROR: ");
  result += msg;
  result += "\n";
  ...
  result += "\n";
  result += "Usage\n";
  result += "   tdcv1x90 create name ?config?\n";
  result += "   tdcv1x90 config name config...\n";
  result += "   tdcv1x90 cget name";
...
*/
int
CV1x90Command::configure(CTCLInterpreter& interp,
