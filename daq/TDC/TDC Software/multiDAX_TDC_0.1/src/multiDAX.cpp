    ...
    while(!quit) {
		...
		if (WriteToFile>0 && FileEvtCounter==-1)
		{
			if (WriteToFile!=3) {
				...
				if (control_openxdio(&xfile,&xatt, &xatt_tdc, OutFileName,FileCounter,WriteToFile,
									 adc,tdc, logfile,EventsPerFile,t)<0) goto exit_prog;
			}
			FileCounter++;
			FileEvtCounter=0;
		}
		...
		if (dataReady)
		{
			tdc_clear_data_buffer(tdc, tdcDataBuffer, bytesCopied_tdc);
			//tdc_enable(crt, tdc);
			// may need to add a first event clear
			usleep(10);
			bytesCopied_tdc = tdc_mblt(crt, tdc, tdcDataBuffer);
			//printf("\nBytes copied: %d", bytesCopied_tdc);
			//printf("\nOutput Data 0x%04X 0x%04X 0x%04X 0x%04X 0x%04X 0x%04X\n", tdcDataBuffer[0], tdcDataBuffer[1], tdcDataBuffer[2], tdcDataBuffer[3], tdcDataBuffer[4], tdcDataBuffer[5]);
			//printf("Output Data 0x%04X 0x%04X 0x%04X 0x%04X 0x%04X 0x%04X\n", tdcDataBuffer[6], tdcDataBuffer[7], tdcDataBuffer[8], tdcDataBuffer[9], tdcDataBuffer[10], tdcDataBuffer[11]);
				...
				if (!syncProblem)
				{
					for (int i=0; i<adc.NumberOfADCs; i++) { // loop over all boards -- write data
						...
						if (WriteToFile!=3) {
							...
							ret=control_writexdio(adc,tdc,t,i,xfile,xatt,xatt_tdc,WriteToFile,bytes[i],buff[i],tdcDataBuffer);
							if (ret>=0) {
								t.tottrg+=ret; FileEvtCounter+=ret;
							} else goto exit_prog;
						}
