...
//-------------------------------------------------------------------
// open xdio-file to store data (XENON100 format)
					  ...
					  u_int32_t *buff_tdc)	// data to write
{
	...
	if (i==adc.NumberOfADCs-1) {
		...
		// added into openxdio like xatt for first chunk
		/*
		xatt_tdc = xdio_create_chunk_raw0_attributes(tdc.sample_size,
														0, // compression  = 0
														tdc.NbSamplesEvt, // window size of 1 (1 sample)
		...
		*/
		...
		for (int k = 0; k < 4096/32; k++) //check all of malloced memory
		{
			int32_t word = buff_tdc[k];
			if(!word) break; // check if you are finished reading data
			if((word & 0xF8000000 != 0xF8000000) && (word & 0xF0000000 != 0xF0000000)) continue; // skip this part of buffer if it is a header or end of block
			...
			// word acts as expected
			//printf("\nWord for channel %d = %lX\n", k, word);
				
			// replace previous or initialized value
			// need validity check because we throw out out this info - check bit set 2 register
		...
		// try making channel length 32 bits and using multi_data instead of for loop above
		/*
		xdio_attach_multi_data_src(0, // call first channel 0th PMT
									0x111111, // read from 6 channels
									(int8_t *)(data_tdc), // pointer to first 32 bit word
									4*tdc.NbChannels, // 4 bytes per channel * 6 channels
									xchunk_tdc); //chunk handle
		*/
		
		//printf("created tdc chunk\n");
		...
		// add tdc chunk to the event (don't need the i==0 bc this is only done on last module anyways)
		// seems to cause problem as is
		//printf("\nxchunk_tdc = %p\n", xchunk_tdc);
		//pointer to xchunk_tdc exists here
		xdio_add_chunk(xchunk_tdc, xevent);
		
		// test get_chunk
		//xdio_chunk_t *testChunk;
		//testChunk = xdio_get_chunk(0, xevent);
		//printf("\ntestChunk0 = %p\n", testChunk);
		//testChunk = xdio_get_chunk(1, xevent);
		//printf("\ntestChunk1 = %p\n", testChunk);
		// grabs chunk properly here but not in xerawdp
