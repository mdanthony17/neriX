...
#ifdef ENABLE_TRIG
	m_iFirstTrigChannel = -1;
	m_iLastTrigChannel = -1;
#endif
...
XmlConfig::readConfigFile(const string &hConfigFileName)
{
	...
	if(!readLiqSciSettings(pRootNode))
		return false;
	readS1LiqSciIntegralSettings(pRootNode);
#endif
...
XmlConfig::printSettings()
{
	...
	if(m_iProcessingLevel > 0)
	{
		...
		cout
			<< "    TriggerSignal" << endl
			<< "      Channels: " << m_iNbTriggerSignals << endl
			<< "      TrigThresholdHeight: " << m_fTrigSignalThresholdHeight << endl
			<< "      TrigThresholdArea: " << m_fTrigSignalThresholdArea << endl;
#endif
		...
		cout
			...
			<< "      DelayS1: " << m_iS1Delay << endl
			<< "      DelayS1: " << m_iS1DelayTime << endl
...
XmlConfig::readSinglePhotoelectronSettings(const xmlpp::Node* pRootNode)
{
	...
	for(int i = 0; i < (int) hSinglePhotoelectronNodeSet.size(); i++)
	{
		...
		else if(hName == "integral_window")
		{
			m_hSingleIntegralWindowValues = hStream.str();
			parseSingleIntegralWindows(m_hSingleIntegralWindowValues);
		}
...
bool
XmlConfig::readS1LiqSciIntegralSettings(const xmlpp::Node* pRootNode)
{
	stringstream hStream;	

	// get liquid scintillator settings
	const xmlpp::NodeSet hS1LiqSciIntegralNodeSet = pRootNode->find("/xerawdp/s1_liq_sci_integration/*");

	for(int i = 0; i < (int) hS1LiqSciIntegralNodeSet.size(); i++)
	{
		string hName = hS1LiqSciIntegralNodeSet[i]->get_name();
		hStream.str(((xmlpp::ContentNode *) hS1LiqSciIntegralNodeSet[i]->get_children().front())->get_content());

		if(hName == "pmt_channel_to_integrate")
			hStream >> m_iPmtToIntegrate;
		else if(hName == "samples_before_liq_sci_peak")
...
#ifdef ENABLE_TRIG
bool
XmlConfig::readTrigSettings(const xmlpp::Node* pRootNode)
{
	stringstream hStream;	

	// get trigger monitoring settings
	const xmlpp::NodeSet hTrigNodeSet = pRootNode->find("/xerawdp/trigger/*");

	for(int i = 0; i < (int) hTrigNodeSet.size(); i++)
	{
		string hName = hTrigNodeSet[i]->get_name();
		hStream.str(((xmlpp::ContentNode *) hTrigNodeSet[i]->get_children().front())->get_content());

		if(hName == "channels")
		{
			m_hTrigChannels = hStream.str();
			parsePmtString(m_hTrigChannels, m_iFirstTrigChannel, m_iLastTrigChannel);
		}
		else if(hName == "signal_threshold_height")
			hStream >> m_fTrigSignalThresholdHeight;
		else if(hName == "signal_threshold_area")
			hStream >> m_fTrigSignalThresholdArea;
...
XmlConfig::readTacSettings(const xmlpp::Node* pRootNode)
{
	...
	for(int i = 0; i < (int) hTimeToAmplitudeConvertersNodeSet.size(); i++)
	{
		...
		if(hName == "s1_delay")
			hStream >> m_iS1Delay;
		else if(hName == "s1_delay_time")
			hStream >> m_iS1DelayTime;
		else if(hName == "nb_time_to_amplitude_converters")
			hStream >> m_iNbTacs;
		else if(hName == "channels")
		{
...
void
XmlConfig::parseSingleIntegralWindows(const string &hValues)
{
	m_hSingleIntegralTableHeader.clear();
	m_hSingleIntegralTableHeader.push_back("window_start");
	m_hSingleIntegralTableHeader.push_back("window_end");

	m_hSingleIntegralTable.clear();
	...
	while((hColonPos = hValues.find(":", hPos)) != string::npos)
	{
		...
		while(!hTmp.eof())
		{
			map<string, float> hSingleIntegral;

			SingleIntegralTableHeader::iterator pColumn;
			for(pColumn = m_hSingleIntegralTableHeader.begin(); pColumn != m_hSingleIntegralTableHeader.end(); pColumn++)
			{
				float fValue = 0.;
				...
				if(hTmp.fail())
					break;
				hSingleIntegral[*pColumn] = fValue;
			}
			...
			if(!hTmp.fail())
			{
				// store them in the pmt calibration table
				for(int iPmt = iFirstPmt; iPmt <= iLastPmt; iPmt++)
					m_hSingleIntegralTable[iPmt] = hSingleIntegral;
			}
		}
