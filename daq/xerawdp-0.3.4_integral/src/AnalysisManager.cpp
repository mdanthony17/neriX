...
//////////////////////////////////////////////////////////
//// The following part is added by Qing
//// for the reconstruction of CoG position
//// @ 2015-09-03
//////////////////////////////////////////////////////////////
...
{
	int ID=id+1;
	if(ID==1 || ID==3 || ID==9 || ID==11)
		return -0.75;
	else if(ID==2 || ID==4 || ID==10 || ID==12)
		return -0.25;
	else if(ID==5 || ID==7 || ID==13 || ID==15)
		return 0.25;
	else if(ID==6 || ID==8 || ID==14 || ID==16)
		return 0.75;
...
ReturnPMTY(int id)
{
	...
	if(ID==1 || ID==2 || ID==5 || ID==6)
		...
		return 0.25;
	else if(ID==9 || ID==10 || ID==13 || ID==14)
		return -0.25;
...
double 
ReconstructX(const std::vector<float> &pS2 )
{
	if(pS2.size() < 16) return -2;
...
double 
ReconstructY(const std::vector<float> &pS2)
{
	//cout<<"pS2 size: "<<pS2.size()<<endl;
...
std::vector<float> ReconstructXY(const std::vector<float> &pS2)
{
...
AnalysisManager::AnalysisManager(const XmlConfig &hXmlConfig, EventData *pEventData)
{
...
#ifdef ENABLE_TRIG
	m_iFirstTrigChannel = hXmlConfig.getFirstTrigChannel();
	m_iLastTrigChannel = hXmlConfig.getFirstTrigChannel();
	m_fTrigSignalThresholdHeight = hXmlConfig.getTrigSignalThresholdHeight();
	m_fTrigSignalThresholdArea = hXmlConfig.getTrigSignalThresholdArea();
#endif
...
#ifdef ENABLE_TAC
	m_iS1Delay = hXmlConfig.getS1Delay();
	m_iS1DelayTime = hXmlConfig.getS1DelayTime();
	...
	m_pSingleIntegralTable = hXmlConfig.getSingleIntegralTable();
...
AnalysisManager::describeLevel0Tree()
{
	...
	m_pLevel0Branches->push_back(Branch("SingleIntegral", "vector<float>", &m_pEventData->m_pSingleIntegral));
	m_pLevel0Branches->push_back(Branch("SinglePrePeakBaselineAvg", "vector<float>", &m_pEventData->m_pSinglePrePeakBaselineAvg));
	m_pLevel0Branches->push_back(Branch("SinglePostPeakBaselineAvg", "vector<float>", &m_pEventData->m_pSinglePostPeakBaselineAvg));
	m_pLevel0Branches->push_back(Branch("SinglePrePeakBaselineRMS", "vector<float>", &m_pEventData->m_pSinglePrePeakBaselineRMS));
	m_pLevel0Branches->push_back(Branch("SinglePostPeakBaselineRMS", "vector<float>", &m_pEventData->m_pSinglePostPeakBaselineRMS));
...
AnalysisManager::describeLevel1Tree()
{
	...
	m_pLevel1Branches->push_back(Branch("S2sHeightChan", "vector<vector<float> >", &m_pEventData->m_pS2sHeightChan));
	...
	m_pLevel1Branches->push_back(Branch("S1sRawNoiseMedian", "vector<vector<float> >", &m_pEventData->m_pS1sRawNoiseMedian));
	m_pLevel1Branches->push_back(Branch("S1sRawNoiseTrapezoid", "vector<vector<float> >", &m_pEventData->m_pS1sRawNoiseTrapezoid));
	...
	m_pLevel1Branches->push_back(Branch("TrigLength", "vector<int>", &m_pEventData->m_pTrigLength));
#endif
...
AnalysisManager::describeLevel2Tree()
{
	...
	m_pLevel2Branches->push_back(Branch("S1sNoiseMedian", "vector<vector<float> >", &m_pEventData->m_pS1sNoiseMedian));
	m_pLevel2Branches->push_back(Branch("S1sNoiseTrapezoid", "vector<vector<float> >", &m_pEventData->m_pS1sNoiseTrapezoid));
	...
	//added by Qing
	m_pLevel2Branches->push_back(Branch("S2sPosCoG", "vector<vector<float> >", &m_pEventData->m_pS2sPosCoG));
	...
	if(pWaveforms->size())
	{
		...
		for(int iPmt = 1; iPmt < m_iNbPmts+1; iPmt++)
		{
			...
			if(pWaveforms->count(iPmt-1))
			{
				...
				float fIntegralStart = (m_pSingleIntegralTable->find(iPmt)->second).find("window_start")->second;
				float fIntegralEnd = (m_pSingleIntegralTable->find(iPmt)->second).find("window_end")->second;
				...
				//compute integral between time windows
				float fIntegralPeak = 0;
				for(int iSample = fIntegralStart; iSample < fIntegralEnd; iSample++)
				{
					fIntegralPeak+=hPmtWaveform[iSample];
				}
				m_pEventData->m_pSingleIntegral->push_back(fIntegralPeak/fSamplingFrequency*fToElectronConversionFactor);

				//compute pre and post-peak window baselines
				float fPrePeakBaseline = 0;
				float fPrePeakBaselineSquared = 0;
				int iPrePeakCounts = 0;
				for(int iSample = int((fIntegralStart+fIntegralEnd)/2)-30; iSample < int((fIntegralStart+fIntegralEnd)/2)-26; iSample++)
				{
					if(iSample>=iWindowStart)
					{
						fPrePeakBaseline+=hPmtWaveform[iSample];
						fPrePeakBaselineSquared+=pow(hPmtWaveform[iSample],2);
						iPrePeakCounts++;
					}
				}
				if(iPrePeakCounts==0)
					iPrePeakCounts==-1;
				m_pEventData->m_pSinglePrePeakBaselineAvg->push_back(fPrePeakBaseline/iPrePeakCounts/fSamplingFrequency*fToElectronConversionFactor);
				m_pEventData->m_pSinglePrePeakBaselineRMS->push_back(pow(fPrePeakBaselineSquared/iPrePeakCounts,0.5)/fSamplingFrequency*fToElectronConversionFactor);

				float fPostPeakBaseline = 0;
				float fPostPeakBaselineSquared = 0;
				int iPostPeakCounts = 0;
				for(int iSample = int((fIntegralStart+fIntegralEnd)/2)+26; iSample < int((fIntegralStart+fIntegralEnd)/2)+30; iSample++)
				{
					if(iSample<=iWindowStart+iWindowWidth)
					{
						fPostPeakBaseline+=hPmtWaveform[iSample];
						fPostPeakBaselineSquared+=pow(hPmtWaveform[iSample],2);
						iPostPeakCounts++;
					}
				}
				if(iPostPeakCounts==0)
					iPostPeakCounts=-1;
				m_pEventData->m_pSinglePostPeakBaselineAvg->push_back(fPostPeakBaseline/iPostPeakCounts/fSamplingFrequency*fToElectronConversionFactor);
				m_pEventData->m_pSinglePostPeakBaselineRMS->push_back(pow(fPostPeakBaselineSquared/iPostPeakCounts,0.5)/fSamplingFrequency*fToElectronConversionFactor);
			...
			else
			{
				...
				m_pEventData->m_pSingleIntegral->push_back(0);
				m_pEventData->m_pSinglePrePeakBaselineAvg->push_back(0);
				m_pEventData->m_pSinglePrePeakBaselineRMS->push_back(0);
				m_pEventData->m_pSinglePostPeakBaselineAvg->push_back(0);
				m_pEventData->m_pSinglePostPeakBaselineRMS->push_back(0);
			}
		}
	}
}
...
AnalysisManager::findPeaks()
{
	...
	// get vector of waveforms for height
	map<int, Waveform *> *pWaveforms = m_pEventData->m_pWaveforms;
	//const float fSamplingFrequency = pWaveforms->begin()->second->getSamplingFrequency();

	for(pS2Peak = hS2Peaks.begin(); pS2Peak != hS2Peaks.end(); pS2Peak++)
	{
		vector<float> peakHeights;
		peakHeights.resize(m_iNbPmts);
		...
		for(int iPmt = 1; iPmt < m_iNbPmts+1; iPmt++)
		{
			if(pWaveforms->count(iPmt-1))
			{
				float peakHeight = (*pWaveforms)[iPmt-1]->maximum(0, (*pWaveforms)[iPmt-1]->getNbSamples());
				//cout << iPmt << ": " << peakHeight << "\n\n";
				peakHeights[iPmt-1] = peakHeight;
			}
			else
			{
				peakHeights[iPmt-1] = 0;
			}
		}
		m_pEventData->m_pS2sHeightChan->push_back(peakHeights);
	...
	else
	{
		...
		for(int iLiqSci = 0; iLiqSci < m_iNbLiqScis; iLiqSci++)
		{
			if(iLiqSci == iLiqSciWithSignal)
			{
				...
				const Peak *pLiqSciPeak = &(m_pPeakFinder->getLiqSciPeak());
				//cout << "\n1197 - " << iLiqSci << " @ " << pLiqSciPeak->getPosition()  << ": " << (*pLiqSciWaveform)[pLiqSciPeak->getPosition()] << "\n";
...
AnalysisManager::getSignalsRaw()
{
	...
	for(int iPeak = 0; iPeak < m_pEventData->m_iNbS1Peaks; iPeak++)
	{
		...
		vector<float> hS1Raw;
		vector<float> hS1RawNoiseMedian;
		vector<float> hS1RawNoiseTrapezoid;
		...
		for(int iPmt = 1; iPmt < m_iNbPmts+1; iPmt++)
		{
			...
			if(pWaveforms->count(iPmt-1))
			{
				...
				// find left and right edge where noise
				// first occurs for median and trapezoid
				// branches

				// must check if edge is different from 
				// baseline (by checking if next bin
				// is the same) to avoid bias

				// don't check equality between floats - 
				// use epsilon instead
				float epsilonDAC = 1E-6;

				float fFurtherLeftDAC = (*(*pWaveforms)[iPmt-1])[iLeftEdge-1];
				float fLeftEdgeDAC = (*(*pWaveforms)[iPmt-1])[iLeftEdge];
				int iTrackMovementLeft = 0;
				int iMaxMovement = (iRightEdge - iLeftEdge);
				while (fLeftEdgeDAC > (fFurtherLeftDAC-epsilonDAC) && fLeftEdgeDAC < (fFurtherLeftDAC+epsilonDAC)  && fLeftEdgeDAC != 0 && iTrackMovementLeft < iMaxMovement)
				{
					iTrackMovementLeft++;
					fFurtherLeftDAC = fLeftEdgeDAC;
					fLeftEdgeDAC = (*(*pWaveforms)[iPmt-1])[iLeftEdge+iTrackMovementLeft];
				}

				float fFurtherRightDAC = (*(*pWaveforms)[iPmt-1])[iRightEdge+1];
				float fRightEdgeDAC = (*(*pWaveforms)[iPmt-1])[iRightEdge];
				int iTrackMovementRight = 0;
				while (fRightEdgeDAC > (fFurtherRightDAC-epsilonDAC) && fRightEdgeDAC < (fFurtherRightDAC+epsilonDAC) && fRightEdgeDAC < -epsilonDAC && fRightEdgeDAC > epsilonDAC && iTrackMovementRight < iMaxMovement)
				{
					iTrackMovementRight++;
					fFurtherRightDAC = fRightEdgeDAC;
					fRightEdgeDAC = (*(*pWaveforms)[iPmt-1])[iRightEdge-iTrackMovementRight];
				}


				float fNoiseFromMedian = (*pWaveforms)[iPmt-1]->median(iLeftEdge, iRightEdge)*(iRightEdge-iLeftEdge)*fToElectronConversionFactor/1E8;  // divide by sampling frequency like integral
				int iSamplesInWindow = iRightEdge - iLeftEdge - iTrackMovementLeft-iTrackMovementRight;
				int iBinsToAverage = 3;
				if (iSamplesInWindow < 3*iBinsToAverage)
					iBinsToAverage = 1;
				float leftAverage = 0;
				float rightAverage = 0;
				for (int iBinFromEdge = 0; iBinFromEdge < iBinsToAverage; iBinFromEdge++)
				{
					leftAverage += (*(*pWaveforms)[iPmt-1])[iLeftEdge+iBinFromEdge+iTrackMovementLeft];
					rightAverage += (*(*pWaveforms)[iPmt-1])[iRightEdge-iBinFromEdge-iTrackMovementRight];
				}

				leftAverage /= (float)iBinsToAverage;
				rightAverage /= (float)iBinsToAverage;

				float fSlopeNoise = (rightAverage-leftAverage)/(float)(iSamplesInWindow - iBinsToAverage);
				float fNoiseFromTrapezoid = 0.0;
				//cout << fLeftEdgeDAC << "   " << fRightEdge << endl;
				for (int step=0; step < iSamplesInWindow+1; step++)
				{
					//cout << fLeftEdgeDAC + fSlopeNoise*step << endl;
					fNoiseFromTrapezoid += (fLeftEdgeDAC + fSlopeNoise*step);
				}

				fNoiseFromTrapezoid *= fToElectronConversionFactor/1E8;	

				/*
				cout << "raw: " << fS1Raw << endl;
				cout << "noise median: " << fNoiseFromMedian << endl;
				cout << "noise trapezoid: " << fNoiseFromTrapezoid << endl;
				*/

				hS1Raw.push_back(fS1Raw);
				hS1RawNoiseMedian.push_back(fNoiseFromMedian);
				hS1RawNoiseTrapezoid.push_back(fNoiseFromTrapezoid);
			...
			{
				hS1Raw.push_back(0.);
				hS1RawNoiseMedian.push_back(0.);
				hS1RawNoiseTrapezoid.push_back(0.);
				hS1MaxPosition.push_back(INT_MAX);
			}
		}
		...
		m_pEventData->m_pS1sRawNoiseMedian->push_back(hS1RawNoiseMedian);
		m_pEventData->m_pS1sRawNoiseTrapezoid->push_back(hS1RawNoiseTrapezoid);
	...
	*/
	...
	for(int iLiqSci = 0; iLiqSci < m_iNbLiqScis; iLiqSci++)
	{
		...
		if(m_pEventData->getLiqSciPeak(iLiqSci) != -1)
		{
			...
			// cout << iPeak << " " << iRightEdge << " " << m_iLiqSciTailStart << endl;
			// waveform integral is in V*s as measured by the digitizer
			float fExternalGain = 10.;
			float fToElectronConversionFactor = 1./(m_fInputImpedance*fExternalGain*m_fElectricCharge);
			float fLiqSciRaw = pLiqSciWaveform->integrate(iLeftEdge, iRightEdge)*fToElectronConversionFactor;
			float fLiqSciTailRaw = pLiqSciWaveform->integrate(iPeak+4., iRightEdge)*fToElectronConversionFactor;
	...
	if(pWaveforms->count(m_iFirstTrigChannel))
	{
		const Waveform *pTriggerWaveform = (*pWaveforms)[m_iFirstTrigChannel];
		
		int iNbSamples = (*pWaveforms)[m_iFirstTrigChannel]->getNbSamples();
		int startSample = -1;
		int endSample = -1;
		...
		for (int iSample = 0; iSample < iNbSamples; iSample++)
		{
			...
			//printf("Height: %f\n", fTriggerSignalHeight);
			
			if(fTriggerSignalHeight > m_fTrigSignalThresholdHeight)
			{
				if(startSample < 0)
				{
					startSample = iSample;
					m_pEventData->m_pTrigLeftEdge->push_back(startSample);
					if(startSample == iNbSamples - 1) m_pEventData->m_pTrigLength->push_back(1);
				}
				else
				{
					endSample = iSample;
					if(endSample == iNbSamples - 1) m_pEventData->m_pTrigLength->push_back(endSample - startSample);

				}
			}
			else if(fTriggerSignalHeight < m_fTrigSignalThresholdHeight && startSample >= 0)
			{
				endSample = iSample;
				fTriggerSignalArea = pTriggerWaveform->integrate(startSample - 1, endSample + 1);
				m_pEventData->m_pTrigLength->push_back(endSample - startSample);

				// fill in vectors with area and leftedge
				if(fTriggerSignalArea > m_fTrigSignalThresholdArea)
				{
...
AnalysisManager::getSignals()
{
	...
	for(int iPeak = 0; iPeak < m_pEventData->m_iNbS1Peaks; iPeak++)
	{
		...
		vector<float> hPeakS1NoiseMedian;
		vector<float> hPeakS1NoiseTrapezoid;
		...
		for(int iPmt = 1; iPmt < m_iNbPmts+1; iPmt++)
		{
			...
			float fS1RawNoiseMedian = (*m_pEventData->getS1sRawNoiseMedian())[iPeak][iPmt-1];
			float fS1RawNoiseTrapezoid = (*m_pEventData->getS1sRawNoiseTrapezoid())[iPeak][iPmt-1];
			float fGain = (m_pPmtCalibrationTable->find(iPmt)->second).find("gain")->second;


			float fS1 = (fGain != 0.)?(((fS1Raw > 0.)?(fS1Raw):(0.))/fGain):(0.);
			float fS1NoiseMedian = (fGain != 0.)?(fS1RawNoiseMedian/fGain):(0.);
			float fS1NoiseTrapezoid = (fGain != 0.)?(fS1RawNoiseTrapezoid/fGain):(0.);
			...
			//cout << "\n1730 - " << iPmt << " S1: " << bExcludedFromTotalSignal << "\n";
			hPeakS1NoiseMedian.push_back(fS1NoiseMedian);
			hPeakS1NoiseTrapezoid.push_back(fS1NoiseTrapezoid);
		...
		m_pEventData->m_pS1sNoiseMedian->push_back(hPeakS1NoiseMedian);
		m_pEventData->m_pS1sNoiseTrapezoid->push_back(hPeakS1NoiseTrapezoid);
	...
	for(int iPeak = 0; iPeak < m_pEventData->m_iNbS2Peaks; iPeak++)
	{
		...
		for(int iPmt = 1; iPmt < m_iNbPmts+1; iPmt++)
		{
			...
			//cout << "\n1804 - " << iPmt << " S2: " << bExcludedFromTotalSignal << "\n";
	...
	for(pTacHeight = m_pEventData->m_pTacHeight->begin(); pTacHeight != m_pEventData->m_pTacHeight->end(); pTacHeight++)
	{
		...
		if(pLiqSci != m_pEventData->m_pLiqSciPeak->end())
		{
			...
			float fTimeOfFlight = m_iS1Delay*m_iS1DelayTime+pow(-1,m_iS1Delay)*((*pTacHeight)-fTacIntercept)/fTacSlope;
	...
	// get waveform of channel we want
	map<int, Waveform *> *pWaveforms = m_pEventData->m_pWaveforms;

	// iPmtToIntegrate is PMT number (starting from 1)
	// iSamplesBeforeLiqSciPeak
	// iWindowForS1Integral

	vector<int>::iterator pLiqSci;
	pLiqSci = find_if(m_pEventData->m_pLiqSciPeak->begin(), m_pEventData->m_pLiqSciPeak->end(), bind2nd(greater<int>(), 0));
	
	int liqSciPeak = -1;
	for (int i=0; i<m_iNbLiqScis; i++)
	{
		liqSciPeak = (*(m_pEventData->m_pLiqSciPeak))[i];
		if (liqSciPeak > 0) break;
	}
	
	int iLeftEdge = liqSciPeak - m_iSamplesBeforeLiqSciPeak;
	int iRightEdge = iLeftEdge + m_iWindowForS1Integral;
	
	//cout << "\n\nLeft Edge, Right Edge: " << iLeftEdge << ", " << iRightEdge << "\n\n";
	
	float fExternalGain = m_pExternalGainsTable->find(m_iPmtToIntegrate)->second;
	float fToElectronConversionFactor = 1./(m_fInputImpedance*fExternalGain*m_fElectricCharge);
...
AnalysisManager::reconstructPosition()
{
	...
	for(pPeakS2 = m_pEventData->m_pS2s->begin(); pPeakS2 != m_pEventData->m_pS2s->end(); pPeakS2++)
	{
		...
		//added by Qing
		m_pEventData->m_pS2sPosCoG->push_back( ReconstructXY( (*pPeakS2) ) );
		//cout<<"#Mark!"<<endl;
	}
}
