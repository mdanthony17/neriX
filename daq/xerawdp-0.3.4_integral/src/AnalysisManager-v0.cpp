...
AnalysisManager::getSignalsRaw()
{
	...
	for(int iPeak = 0; iPeak < m_pEventData->m_iNbS1Peaks; iPeak++)
	{
		...
		int iS2Trigger = 0;
		int triggerDelay = 2000;//S1 trigger is 20 us behind S1 peak
		...
		if(pWaveforms->count(m_iFirstTriggerEfficiencyChannel))
		{
			...
			float fTriggerSignalHeight = pTriggerWaveform->integrate(iLeftEdge+m_iTriggerWindowStart+triggerDelay, iLeftEdge+m_iTriggerWindowStart+m_iTriggerWindowWidth+triggerDelay);
	...
	for(int iPeak = 0; iPeak < m_pEventData->m_iNbS2Peaks; iPeak++)
	{
		...
		if(pWaveforms->count(m_iFirstTriggerEfficiencyChannel))
		{
			...
			// waveform is in V as measured by the digitizer
			float fTriggerSignalHeight = pTriggerWaveform->maximum(iLeftEdge+m_iTriggerWindowStart, iLeftEdge+m_iTriggerWindowStart+m_iTriggerWindowWidth);
	...
	if(pWaveforms->count(18))
	{
		...
		float triggerThreshold = 0.; // fill in threshold using cout below
		int iNbSamples = (*pWaveforms)[18]->getNbSamples();
		int startSample = -1;
		int endSample = -1;
		float fTriggerSignalArea;
		for (int iSample = 0; i < iNbSamples; iSample++)
		{
	...
	for(int iPeak = 0; iPeak < m_pEventData->m_iNbS1Peaks; iPeak++)
	{
		...
		for(int iPmt = 1; iPmt < m_iNbPmts+1; iPmt++)
		{
			...
			if((isTopPmt(iPmt) || isBottomPmt(iPmt)) && !bExcludedFromTotalSignal)
			{
				...
				//extend range of pmts to include trigger and then make similar
				//else if
			}
			else
				hPeakS1.push_back(0.);
