...
double g1 = 0.1075; //phd per S1 photon in liquid at dtCntr (not phe)
double sp = 1.32; //single phe resolution (Gaussian assumed)
double ee = 0.935; //e- extraction efficiency, overriding estimate from field
double g1_gas = 0.0425; //phd per S2 photon in gas, used to get SE size - what is this for nerix??
...
int nFold = 1; //how many PMTs have to fire for an S1 to count
...
double dtCntr = 9.4; //center of detector for S1 corrections, in usec.
double dt_min = 2.48; //minimum. Top of detector fiducial
double dt_max = 16.6; //maximum. Bottom of detector fiducial
...
double gasGap = 0.1; //EL gap in cm, effecting both field and linear S2 term
...
int numberPMTs = 1; //in LUX, 122 minus 3 de-activated. For coincidence
