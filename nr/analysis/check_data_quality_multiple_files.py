...
run1 = neriX_analysis.neriX_analysis(neriX_analysis.pull_all_files_given_parameters(run, anodeSetting, cathodeSetting, degreeSetting), degreeSetting, cathodeSetting, anodeSetting)

file1 = '%d deg, %.3f kV' % (degreeSetting, cathodeSetting)
...
run1.add_cut('S1sTotBottom[0] > 0')
#run1('S2sTotBottom[0] > 0')
run1.add_z_cut()
run1.add_radius_cut(0, 0.85)
#run1.add_s1_liqsci_peak_cut()
run1.add_s1_asym_cut()
#run1.add_temp_neutron_cut(degreeSetting)
#run1.add_temp_tof_cut(degreeSetting)
...
ha13 = Hist2D(150, 0, 35, 150, 1, 5, name='ha13', title='NR and ER bands - ' + file1)
