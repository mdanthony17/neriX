...
for i in tqdm(xrange(numEvents)):
	...
	for peakNumber, peakTime in enumerate(lS1sPeak):
		...
		hNoCut.Fill(lS1sTotBottom[peakNumber])
		#if peakNumber > 0:
		#	continue
		if lS1sPeak[peakNumber] > lS2sLeftEdge[0]:
			continue
		...
		for trigNumber, trigTime in enumerate(lTrigLeftEdge):
			...
			if (trigTime - peakTime) > lowTimeDiff and (trigTime - peakTime) < highTimeDiff:
				...
				hWithCut.Fill(lS1sTotBottom[peakNumber])
...
sTriggerCut = '(TrigLeftEdge[] - S1sPeak[S1Order[0]]) > %f && (TrigLeftEdge[] - S1sPeak[S1Order[0]]) < %f && cpS1sTotBottom[S1Order[0]] > 0.5' % (lowTimeDiff, highTimeDiff)
...
hEff.SetTitle('S1 Efficiency for TAC; cpS1sTotBottom[S1Order[0]] [PE]; Percentage Causing Discriminator Pulse for TAC');
