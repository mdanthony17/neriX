...
leftEdgeUpperBound = 6

rightEdgeLowerBound = 8
rightEdgeUpperBound = 46

closestS1S2Distance = 300
...
for i in tqdm(xrange(numEvents)):
	...
	lS1sTot = list(current_analysis.get_T2().S1sTot)
	...
	#if lS1sTotBottom[lS1Order[0]] < 50 or lS1sTotBottom[lS1Order[0]] > 100:
	#	continue

	# make sure no close S1s
	if len(lS1sPeak) > 1 and lS1sPeak[lS1Order[1]] - lS1sPeak[lS1Order[0]] < closestS1S2Distance:
		continue

	# make sure no close S2s
	if lS2sLeftEdge[lS2Order[0]] - lS1sPeak[lS1Order[0]] < closestS1S2Distance:
		continue

	if (lS1sTotBottom[0]-lS1sTotTop[0])/lS1sTot[0] < -1.4*exp(-lS1sTotBottom[0]/10.)+0.4:
		continue
	
	# cut out noise triggers
	noiseTriggerFound = False
	for trigNumber, trigTime in enumerate(lTrigLeftEdge):
		timeDiffLeft = trigTime - lS1sPeak[lS1Order[0]]
		if timeDiffLeft > -80 and timeDiffLeft < 0:
			noiseTriggerFound = True

	if noiseTriggerFound:
		continue

	hNoCut.Fill(lS1sTotBottom[lS1Order[0]])
	
	# find closest trigger
	trigFound = -1
	for trigNumber, trigTime in enumerate(lTrigLeftEdge):
		timeDiffLeft = trigTime - lS1sPeak[lS1Order[0]]
		timeDiffRight = trigTime + lTrigLength[trigNumber] - lS1sPeak[lS1Order[0]]

		#if (lowTimeDiff < timeDiffLeft < highTimeDiff) or (rightEdgeLowerBound < timeDiffRight < rightEdgeUpperBound):
		if timeDiffLeft < leftEdgeUpperBound and timeDiffRight > rightEdgeLowerBound:
			trigFound = trigNumber
			hWithCut.Fill(lS1sTotBottom[lS1Order[0]])
			hGaus.Fill(timeDiffLeft)
			break


	#if lS1sTotBottom[lS1Order[0]] > 15 and trigFound == -1:
	#	print i, lTrigLeftEdge[trigFound], lS1sLeftEdge[lS1Order[0]]

	#if closestTrigger > 10 and closestTrigger < 30:
