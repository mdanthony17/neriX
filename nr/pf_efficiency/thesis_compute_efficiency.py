...
gain_adjustment_term = 1.4e6/8.68e5
...
h_pf_efficiency.SetTitle('; Simulated PE [PE]; Percentage Found by Peak Finder');
...
def pyfunc_eff(x, center, shape):
	return 1./(1. + exp(-(x-center)/shape))

g_eff_conf_band = neriX_analysis.create_1d_fit_confidence_band(pyfunc_eff, a_fit_pars, a_cov_matrix, s1_settings_small_window[1], s1_settings_small_window[2], confidence_level=0.68, root_output=True)
...
tpt_pf_efficiency.AddText('#varepsilon_{S1 PF} = #left( 1 + e^{ #frac{-(S1-#alpha)}{#beta} } #right)^{-1}')
