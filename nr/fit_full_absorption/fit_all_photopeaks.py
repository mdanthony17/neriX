...
if len(sys.argv) != 7:
	print 'Use is python fit_photopeak_radial_cut.py <run> <degree setting> <cathode setting> <anode setting> <radial cut> <number of processors>'
	sys.exit()

run = int(sys.argv[1])
degreeSetting = int(sys.argv[2])
cathodeSetting = float(sys.argv[3])
anodeSetting = float(sys.argv[4])

radialCut = float(sys.argv[5])

numProcessors = int(sys.argv[6])
...
for file in dDatasets[run]:
	if dDatasets[run][file] == (anodeSetting, cathodeSetting, degreeSetting):
		lFilesToLoad.append(file)

currentAnalysis = neriX_analysis(lFilesToLoad, degreeSetting, cathodeSetting, anodeSetting)
...
try:
	dCompletedFitsHistParameters = pickle.load(open('completed_fits__radial_cut_hist_parameters.p', 'r'))
except:
...
if degree > -1:
	...
	print 'Only setup to handle coincidence and minitron calibrations (degree = -4)'
	sys.exit()
...
tParameters = (degreeSetting, cathodeSetting, anodeSetting)
cuts = currentAnalysis.get_cuts()

# load old parameters if possible
useOldParameters = False
try:
	dCompletedFitsHistParameters[tParameters]
	...
	if string.lower(responseOldFit) == 'y':
		dHistParameters = dCompletedFitsHistParameters[tParameters]
		dEllipse = set_ellipse_parameters(dHistParameters)
...
while string.lower(responseSatisfied) != 'y' or fitStatus != 3:
	...
	#currentAnalysis.Draw('ctS1sTotBottom[0]:S2sTotTop[0]', hist=h1, selection=cuts)
	currentAnalysis.multithread_draw(numProcessors, 'S1sTotBottom[0]:S2sTotBottom[0]', hist=h1, selection=cuts)
...
dParametersToPrint = {'run':currentAnalysis.get_run(), 'anode':currentAnalysis.get_anode_setting(), 'cathode':currentAnalysis.get_cathode_setting(), 'degree':currentAnalysis.get_degree_setting(), 's1_mean':f1.GetParameter(1), 's1_mean_err':f1.GetParError(1), 's1_sigma':f1.GetParameter(4), 's1_sigma_err':f1.GetParError(4), 's2_mean':f1.GetParameter(3), 's2_mean_err':f1.GetParError(3), 's2_sigma':f1.GetParameter(5), 's2_sigma_err':f1.GetParError(5), 'Chi2':fitResult.Chi2(), 'NDF':fitResult.Ndf()}

print '\n\nPlease copy below into fits_completed.py\n'
print '#' + str(tParameters)
print 'dFitParameters[\'' + str(tParameters) + '\'] =', dParametersToPrint
print '\n\n'

dCompletedFitsHistParameters[tParameters] = dHistParameters
pickle.dump(dCompletedFitsHistParameters, open('completed_fits__radial_cut_hist_parameters.p', 'w'))
