...
d_s1_cuts[3000] = [0, 10, 20, 40]
d_s1_cuts[3500] = [0, 10, 20, 40]
...
l_s2_settings_band = [30, 1000, 6000]
...
for cathode_setting in d_bin_edges['s1']:
    d_gpu_bin_edges['s1'][cathode_setting] = {}
    d_gpu_bin_edges['log'][cathode_setting] = {}
    for degree_setting in d_bin_edges['s1'][cathode_setting]:
        d_gpu_bin_edges['s1'][cathode_setting][degree_setting] = pycuda.gpuarray.to_gpu(d_bin_edges['s1'][cathode_setting][degree_setting])
        d_gpu_bin_edges['log'][cathode_setting][degree_setting] = pycuda.gpuarray.to_gpu(d_bin_edges['log'][cathode_setting][degree_setting])
"""
...
for cathode_setting in l_cathode_settings_in_use:
    ...
    for degree_setting in l_degree_settings_in_use:
        ...
        for i in tqdm.tqdm(xrange(num_mc_events)):
            ...
            if degree_setting == -4:
                while random_energy < (d_bin_edges['s1'][0.345][-4][0]*0.25/0.11/7.):
                    random_energy = h_energy.GetRandom()
...
for cathode_setting in l_cathode_settings_in_use:
    ...
    for degree_setting in l_degree_settings_in_use:
        ...
        g1_value = np.asarray(a_best_fit_parameters[7], dtype=np.float32)
        spe_res = np.asarray(a_best_fit_parameters[8], dtype=np.float32)
        extraction_efficiency = np.asarray(a_best_fit_parameters[9], dtype=np.float32)
        gas_gain_value = np.asarray(a_best_fit_parameters[10], dtype=np.float32)
        gas_gain_width = np.asarray(a_best_fit_parameters[11], dtype=np.float32)

        intrinsic_res_s1 = np.asarray(a_best_fit_parameters[5], dtype=np.float32)
        intrinsic_res_s2 = np.asarray(a_best_fit_parameters[6], dtype=np.float32)

        pf_eff_par0 = np.asarray(a_best_fit_parameters[12], dtype=np.float32)
        pf_eff_par1 = np.asarray(a_best_fit_parameters[13], dtype=np.float32)

        s2_eff_par0 = np.asarray(a_best_fit_parameters[14], dtype=np.float32)
        s2_eff_par1 = np.asarray(a_best_fit_parameters[15], dtype=np.float32)

        a_pf_stdev = np.asarray([a_best_fit_parameters[16], a_best_fit_parameters[17], a_best_fit_parameters[18]], dtype=np.float32)
        
        dpe_prob = np.asarray(a_best_fit_parameters[19], dtype=np.float32)
        ...
        ti_par = np.asarray(a_best_fit_parameters[19+1+count_fields*num_degree_field_pars], dtype=np.float32)
    
        exciton_to_ion_ratio = np.asarray(a_best_fit_parameters[19+2+count_fields*num_degree_field_pars], dtype=np.float32)
    
        if degree_setting == -4:
            bkg_prob = np.asarray(0., dtype=np.float32)
            scale_par = np.asarray(a_best_fit_parameters[19+3+count_fields*num_degree_field_pars], dtype=np.float32)
        else:
            bkg_prob = np.asarray(a_best_fit_parameters[19+4+count_fields*num_degree_field_pars + 2*count_degrees], dtype=np.float32)
            scale_par = np.asarray(a_best_fit_parameters[19+5+count_fields*num_degree_field_pars + 2*count_degrees], dtype=np.float32)
    
  

        tArgs = (local_rng_states, drv.In(num_trials), drv.In(mean_field), d_plotting_information[cathode_setting][degree_setting]['gpu_energy'], gpu_bkg_energy, drv.In(w_value), drv.In(exciton_to_ion_ratio), drv.In(ti_par), drv.In(beta), drv.In(kappa), drv.In(eta), drv.In(lamb), drv.In(g1_value), drv.In(extraction_efficiency), drv.In(gas_gain_value), drv.In(gas_gain_width), drv.In(spe_res), drv.In(intrinsic_res_s1), drv.In(intrinsic_res_s2), drv.In(a_pf_stdev), drv.In(pf_eff_par0), drv.In(pf_eff_par1), drv.In(s2_eff_par0), drv.In(s2_eff_par1), drv.In(dpe_prob), drv.In(bkg_prob), drv.InOut(a_s1_mc), drv.InOut(a_s2_mc), drv.InOut(a_weight))
        ...
        if degree_setting == -4:
            s2_edges = np.linspace(l_s2_settings_band[1], l_s2_settings_band[2], l_s2_settings_band[0]+1)
        else:
        ...
        # find the chi2
        indices_for_log_likelihood = (hist_s1_log_data[0] > 3)
        ...
        for i in tqdm.tqdm(xrange(num_walkers*num_steps)):
            ...
            g1_value = np.asarray(a_fit_parameters[7], dtype=np.float32)
            spe_res = np.asarray(a_fit_parameters[8], dtype=np.float32)
            extraction_efficiency = np.asarray(a_fit_parameters[9], dtype=np.float32)
            gas_gain_value = np.asarray(a_fit_parameters[10], dtype=np.float32)
            gas_gain_width = np.asarray(a_fit_parameters[11], dtype=np.float32)

            intrinsic_res_s1 = np.asarray(a_fit_parameters[5], dtype=np.float32)
            intrinsic_res_s2 = np.asarray(a_fit_parameters[6], dtype=np.float32)

            pf_eff_par0 = np.asarray(a_fit_parameters[12], dtype=np.float32)
            pf_eff_par1 = np.asarray(a_fit_parameters[13], dtype=np.float32)

            s2_eff_par0 = np.asarray(a_fit_parameters[14], dtype=np.float32)
            s2_eff_par1 = np.asarray(a_fit_parameters[15], dtype=np.float32)

            a_pf_stdev = np.asarray([a_fit_parameters[16], a_fit_parameters[17], a_fit_parameters[18]], dtype=np.float32)
            
            dpe_prob = np.asarray(a_fit_parameters[19], dtype=np.float32)

            ti_par = np.asarray(a_fit_parameters[19+1+count_fields*num_degree_field_pars], dtype=np.float32)
        
            exciton_to_ion_ratio = np.asarray(a_fit_parameters[19+2+count_fields*num_degree_field_pars], dtype=np.float32)
        
            if degree_setting == -4:
                bkg_prob = np.asarray(0., dtype=np.float32)
                scale_par = np.asarray(a_fit_parameters[19+3+count_fields*num_degree_field_pars], dtype=np.float32)
            else:
                bkg_prob = np.asarray(a_fit_parameters[19+4+count_fields*num_degree_field_pars + 2*count_degrees], dtype=np.float32)
                scale_par = np.asarray(a_fit_parameters[19+5+count_fields*num_degree_field_pars + 2*count_degrees], dtype=np.float32)
            ...
            a_weight = np.full(num_mc_events, 0, dtype=np.float32)

            tArgs = (local_rng_states, drv.In(num_trials), drv.In(mean_field), d_plotting_information[cathode_setting][degree_setting]['gpu_energy'], gpu_bkg_energy, drv.In(w_value), drv.In(exciton_to_ion_ratio), drv.In(ti_par), drv.In(beta), drv.In(kappa), drv.In(eta), drv.In(lamb), drv.In(g1_value), drv.In(extraction_efficiency), drv.In(gas_gain_value), drv.In(gas_gain_width), drv.In(spe_res), drv.In(intrinsic_res_s1), drv.In(intrinsic_res_s2), drv.In(a_pf_stdev), drv.In(pf_eff_par0), drv.In(pf_eff_par1), drv.In(s2_eff_par0), drv.In(s2_eff_par1), drv.In(dpe_prob), drv.In(bkg_prob), drv.InOut(a_s1_mc_current_iteration), drv.InOut(a_s2_mc_current_iteration), drv.InOut(a_weight))
            ...
            scaling_factor_for_histogram = scale_par*data_integral/mc_integral
        ...
        fig_s1_log.savefig('%ss_data_mc_comparison_s1_and_2d_%s_%.3f_kV_%d_deg.png' % (s_path_for_save, dir_specifier_name, cathode_setting, degree_setting))
        fig_s2s.savefig('%ss_data_mc_comparison_s2_slices_%s_%.3f_kV_%d_deg.png' % (s_path_for_save, dir_specifier_name, cathode_setting, degree_setting))
