...
"""
# KDE
print '\n\nUsing marginalized mode in instead of median\n\n'

num_dim = a_full_sampler.shape[2]
a_best_fit_parameters = np.zeros(num_dim)
        
max_num_events_for_kde = 5e4
assert num_steps*num_walkers < max_num_events_for_kde, 'Using KDE to estimate maximum in full space so must use less than %d events for time constraints.\n' % (int(max_num_events_for_kde))

a_partial_sampler = np.zeros((num_steps*num_walkers, num_dim))
for i in tqdm.tqdm(xrange(num_steps*num_walkers)):
    a_partial_sampler[i, :] = a_samples[-i, :]
...
for i in xrange(num_dim):
    ...
    grid = grid_search.GridSearchCV(neighbors.KernelDensity(), {'bandwidth':np.linspace(0.01, 2., 40)}, cv=4, verbose=1, n_jobs=1)
    ...
    def func_for_minimizing_for_plot(a_parameters):
        a_scaled_parameters = scaler.transform(a_parameters.reshape(-1, 1))
    ...
    a_best_fit_parameters[i] = result.x[0]
...
for cathode_setting in l_cathode_settings_in_use:
    ...
    for degree_setting in l_degree_settings_in_use:
        ...
        """
        num_data_pts_per_dataset = 500#len(current_df_data['s1'])
        num_resampled_datasets = int(len(df_mc['s1']) / num_data_pts_per_dataset)
        print 'Number of resampled datasets and bootstraps: %d' % (num_resampled_datasets)
        l_resampled_datasets = []
        for i in xrange(num_resampled_datasets):
            idx_data = np.random.choice(len(current_df_data['s1']), num_data_pts_per_dataset, replace=True)
            l_resampled_datasets.append([current_df_data['s1'].values[idx_data], current_df_data['log'].values[idx_data]])
        
        num_to_choose = 1000#len(current_df_data['s1'])
        if len(df_mc['s1']) < num_to_choose:
            num_to_choose = len(df_mc['s1'])
        ...
        idx_mc = np.random.choice(len(a_s1_mc), num_to_choose, replace=True, p=a_probs)
        
        a_data_ds = np.zeros(num_resampled_datasets)
        
        for i, l_x in enumerate(l_resampled_datasets):
            a_data_s1, a_data_log = l_x
            p_value_data, D_data = pff_test.ks2d2s(a_s1_mc[idx_mc], a_log_mc[idx_mc], a_data_s1, a_data_log, extra=True)
            #print D_data
            a_data_ds[i] = D_data
            
        D_data_median = np.median(a_data_ds)
        print 'D data: %f\n\n\n' % (D_data_median)
        
        
        # simulate datasets
        num_bootstraps = num_resampled_datasets
        a_mc_ds = np.zeros(num_bootstraps)
        for i in xrange(num_bootstraps):
            idx_bootstrap = np.random.choice(len(a_s1_mc), num_data_pts_per_dataset, replace=True, p=a_probs)
            p_value_mc, D_mc = pff_test.ks2d2s(a_s1_mc[idx_mc], a_log_mc[idx_mc], a_s1_mc[idx_bootstrap], a_log_mc[idx_bootstrap], extra=True)
            #print D_mc
            a_mc_ds[i] = D_mc
        
        p_value_tot = np.sum(a_mc_ds > D_data_median) / float(num_bootstraps)
        print np.mean(a_mc_ds), np.std(a_mc_ds)
        print 'pvalue from bootstrap: %f\n\n' % (p_value_tot)
        """
        
        

        #plt.scatter(np.asarray(current_df_data['s1']), np.asarray(current_df_data['log']), color='r', marker='.')
        #plt.hist2d(a_s1_mc[idx], a_log_mc[idx])
        #plt.show()
