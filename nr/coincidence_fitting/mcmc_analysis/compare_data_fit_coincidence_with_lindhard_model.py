...
dir_specifier_name = 'multiple_energies_lindhard_model'


l_degree_settings_in_use = [3000, 3500, 4500, 5300]
s_degree_settings = ''
...
gpu_observables_func = SourceModule(cuda_full_observables_production.cuda_full_observables_production_code, no_extern_c=True).get_function('gpu_full_observables_production_with_arrays_gompertz_wth_bkg')


num_field_degree_pars = len(l_cathode_settings_in_use)*(5) + len(l_degree_settings_in_use)*len(l_cathode_settings_in_use)*2
...
for cathode_setting in l_cathode_settings_in_use:
    ...
    for degree_setting in l_degree_settings_in_use:
        ...
        for quantile_number, a_fit_parameters in enumerate(a_percentile_values):
            ...
            gom_par0 = np.asarray(a_fit_parameters[-num_field_degree_pars + 5*count_fields + 0], dtype=np.float32)
            gom_par1 = np.asarray(a_fit_parameters[-num_field_degree_pars + 5*count_fields + 1], dtype=np.float32)
            gom_par2 = np.asarray(a_fit_parameters[-num_field_degree_pars + 5*count_fields + 2], dtype=np.float32)
            gom_par3 = np.asarray(a_fit_parameters[-num_field_degree_pars + 5*count_fields + 3], dtype=np.float32)
        
            exciton_to_ion_ratio = np.asarray(a_fit_parameters[-num_field_degree_pars + 5*count_fields + 4], dtype=np.float32)
        
            bkg_prob = np.asarray(a_fit_parameters[-num_field_degree_pars + 5*count_fields + 5 + 2*count_degrees], dtype=np.float32)
            scale_par = np.asarray(a_fit_parameters[-num_field_degree_pars + 5*count_fields + 6 + 2*count_degrees], dtype=np.float32)
        
      

            tArgs = (local_rng_states, drv.In(num_trials), drv.In(mean_field), d_plotting_information[cathode_setting][degree_setting]['gpu_energy'], gpu_bkg_energy, drv.In(w_value), drv.In(exciton_to_ion_ratio), drv.In(gom_par0), drv.In(gom_par1), drv.In(gom_par2), drv.In(gom_par3), drv.In(kappa), drv.In(g1_value), drv.In(extraction_efficiency), drv.In(gas_gain_value), drv.In(gas_gain_width), drv.In(spe_res), drv.In(intrinsic_res_s1), drv.In(intrinsic_res_s2), drv.In(a_pf_stdev), drv.In(pf_eff_par0), drv.In(pf_eff_par1), drv.In(s2_eff_par0), drv.In(s2_eff_par1), drv.In(dpe_prob), drv.In(bkg_prob), drv.InOut(a_s1_mc), drv.InOut(a_s2_mc))
        ...
        for i in tqdm.tqdm(xrange(num_walkers*num_steps)):
            ...
            tArgs = (local_rng_states, drv.In(num_trials), drv.In(mean_field), d_plotting_information[cathode_setting][degree_setting]['gpu_energy'], gpu_bkg_energy, drv.In(w_value), drv.In(exciton_to_ion_ratio), drv.In(gom_par0), drv.In(gom_par1), drv.In(gom_par2), drv.In(gom_par3), drv.In(kappa), drv.In(g1_value), drv.In(extraction_efficiency), drv.In(gas_gain_value), drv.In(gas_gain_width), drv.In(spe_res), drv.In(intrinsic_res_s1), drv.In(intrinsic_res_s2), drv.In(a_pf_stdev), drv.In(pf_eff_par0), drv.In(pf_eff_par1), drv.In(s2_eff_par0), drv.In(s2_eff_par1), drv.In(dpe_prob), drv.In(bkg_prob), drv.InOut(a_s1_mc_current_iteration), drv.InOut(a_s2_mc_current_iteration))
