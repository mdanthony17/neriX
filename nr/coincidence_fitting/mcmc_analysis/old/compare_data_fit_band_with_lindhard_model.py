...
gpu_observables_func = SourceModule(cuda_full_observables_production.cuda_full_observables_production_code, no_extern_c=True).get_function('gpu_full_observables_production_lindhard')
...
for cathode_setting in l_cathode_settings_in_use:
    for degree_setting in l_degree_settings_in_use:
        ...
        for quantile_number, a_fit_parameters in enumerate(a_percentile_values):
            ...
            spe_res = np.asarray(a_fit_parameters[10], dtype=np.float32)
            extraction_efficiency = np.asarray(a_fit_parameters[11], dtype=np.float32)
            gas_gain_value = np.asarray(a_fit_parameters[12], dtype=np.float32)
            gas_gain_width = np.asarray(a_fit_parameters[13], dtype=np.float32)

            intrinsic_res_s1 = np.asarray(a_fit_parameters[7], dtype=np.float32)
            intrinsic_res_s2 = np.asarray(a_fit_parameters[8], dtype=np.float32)

            pf_eff_par0 = np.asarray(a_fit_parameters[14], dtype=np.float32)
            pf_eff_par1 = np.asarray(a_fit_parameters[15], dtype=np.float32)

            s2_eff_par0 = np.asarray(a_fit_parameters[16], dtype=np.float32)
            s2_eff_par1 = np.asarray(a_fit_parameters[17], dtype=np.float32)

            a_pf_stdev = np.asarray([a_fit_parameters[18], a_fit_parameters[19], a_fit_parameters[20]], dtype=np.float32)

            a_s1_mc = np.zeros(num_mc_events, dtype=np.float32)
            a_s2_mc = np.zeros(num_mc_events, dtype=np.float32)


            tArgs = (local_rng_states, drv.In(num_trials), drv.In(mean_field), d_plotting_information[cathode_setting][degree_setting]['gpu_energy'], drv.In(w_value), drv.In(alpha), drv.In(zeta), drv.In(beta), drv.In(gamma), drv.In(delta), drv.In(kappa), drv.In(g1_value), drv.In(extraction_efficiency), drv.In(gas_gain_value), drv.In(gas_gain_width), drv.In(spe_res), drv.In(intrinsic_res_s1), drv.In(intrinsic_res_s2), drv.In(a_pf_stdev), drv.In(pf_eff_par0), drv.In(pf_eff_par1), drv.In(s2_eff_par0), drv.In(s2_eff_par1), drv.In(np.asarray(1, dtype=np.int32)), drv.InOut(a_s1_mc), drv.InOut(a_s2_mc))
        ...
        for i in tqdm.tqdm(xrange(num_walkers*num_steps)):
            ...
            tArgs = (local_rng_states, drv.In(num_trials), drv.In(mean_field), d_plotting_information[cathode_setting][degree_setting]['gpu_energy'], drv.In(w_value), drv.In(alpha), drv.In(zeta), drv.In(beta), drv.In(gamma), drv.In(delta), drv.In(kappa), drv.In(g1_value), drv.In(extraction_efficiency), drv.In(gas_gain_value), drv.In(gas_gain_width), drv.In(spe_res), drv.In(intrinsic_res_s1), drv.In(intrinsic_res_s2), drv.In(a_pf_stdev), drv.In(pf_eff_par0), drv.In(pf_eff_par1), drv.In(s2_eff_par0), drv.In(s2_eff_par1), drv.In(np.asarray(1, dtype=np.int32)), drv.InOut(a_s1_mc_current_iteration), drv.InOut(a_s2_mc_current_iteration))
