...
class fit_nr(object):
    def __init__(self, fit_type, d_coincidence_data, d_band_data=None, free_exciton_to_ion_ratio=False, num_mc_events=5e6, name_notes=None, l_gpus=[0], num_loops=1, test_switch='', small_number=0.001):
        ...
        self.l_means_pf_stdev_pars = [0.035308831318021296, 0.014675238121911261, 0.5283368131669588, 6.9731568575515102]
        self.l_cov_matrix_pf_stdev_pars = [[9.3091740626955799e-10, 4.2846233221300459e-11, 1.5670891684587857e-09, -2.4830392976389202e-08], [4.2846233221300459e-11, 9.7679768145661727e-10, 2.163163938286531e-08, -3.8945450018151318e-07], [1.5670891684587857e-09, 2.163163938286531e-08, 4.8569294796130197e-06, -2.9829801702396074e-05], [-2.4830392976389202e-08, -3.8945450018151318e-07, -2.9829801702396074e-05, 0.000347538069496915]]
        ...
        if fit_type == 's':
            ...
            if fit_type == 'ml':
                ...
                self.num_dimensions = 20 + len(self.l_cathode_settings_in_use)*len(self.l_degree_settings_in_use)*2 + len(self.l_cathode_settings_in_use)*(1+1) - len(self.l_cathode_settings_in_use) + 2 # 17 + scales + bkg + recombination + exciton-to-ion ratio
    ...
    def fill_energy_array(self, view_energy_spectrum=False):
        ...
        for degree_setting in self.l_degree_settings_in_use:
            ...
            for i in tqdm.tqdm(xrange(self.num_mc_events)):
                ...
                if degree_setting == -4:
                    while random_energy < (self.d_bin_edges['s1'][0.345][-4][0]*0.25/0.11/7.):
                        random_energy = self.d_energy_arrays[degree_setting]['h_energy'].GetRandom()
    ...
    def get_pf_stdev_default(self, pf_stdev_par0, pf_stdev_par1, pf_stdev_par2, pf_stdev_par3):
        return multivariate_normal.pdf([pf_stdev_par0, pf_stdev_par1, pf_stdev_par2, pf_stdev_par3], self.l_means_pf_stdev_pars, self.l_cov_matrix_pf_stdev_pars), pf_stdev_par0, pf_stdev_par1, pf_stdev_par2, pf_stdev_par3
    ...
    def ln_likelihood_full_matching_single_energy(self, py, qy, intrinsic_res_s1, intrinsic_res_s2, g1_value, spe_res_value, extraction_efficiency_value, gas_gain_mean_value, gas_gain_width_value, pf_eff_par0, pf_eff_par1, s2_eff_par0, s2_eff_par1, pf_stdev_par0, pf_stdev_par1, pf_stdev_par2, pf_stdev_par3, exciton_to_ion_par0_rv, exciton_to_ion_par1_rv, exciton_to_ion_par2_rv, prob_bkg_event, scale_par, d_gpu_local_info, draw_fit=False):
        ...
        current_ln_likelihood, pf_stdev_par0, pf_stdev_par1, pf_stdev_par2, pf_stdev_par3 = self.get_pf_stdev_default(pf_stdev_par0, pf_stdev_par1, pf_stdev_par2, pf_stdev_par3)
        ...
        a_pf_stdev = np.asarray([pf_stdev_par0, pf_stdev_par1, pf_stdev_par2, pf_stdev_par3], dtype=np.float32)
    ...
    def ln_likelihood_full_matching_multiple_energies_lindhard_model_with_ti_recombination(self, w_value, beta, kappa, eta, lamb, intrinsic_res_s1, intrinsic_res_s2, g1_value, spe_res_value, extraction_efficiency_value, gas_gain_mean_value, gas_gain_width_value, pf_eff_par0, pf_eff_par1, s2_eff_par0, s2_eff_par1, pf_stdev_par0, pf_stdev_par1, pf_stdev_par2, pf_stdev_par3, dpe_prob, d_field_degree_pars, d_gpu_local_info, draw_fit=False):
        ...
        for cathode_setting in self.l_cathode_settings_in_use:
            for degree_setting in self.l_degree_settings_in_use:
                ...
                # get resolution
                if degree_setting == -4:
                    intrinsic_res_s1 = np.asarray(d_field_degree_pars[cathode_setting][degree_setting]['band_res_s1'], dtype=np.float32)
                    intrinsic_res_s2 = np.asarray(d_field_degree_pars[cathode_setting][degree_setting]['band_res_s2'], dtype=np.float32)
                else:
                    intrinsic_res_s1 = np.asarray(intrinsic_res_s1, dtype=np.float32)
                    intrinsic_res_s2 = np.asarray(intrinsic_res_s2, dtype=np.float32)
    ...
    def wrapper_ln_likelihood_full_matching_multiple_energies_lindhard_model_with_ti_recombination(self, a_parameters, kwargs={}):
        ...
        if -4 in self.l_degree_settings_in_use:
            ...
            # add in resolution parameters
            num_field_degree_pars += 2
        else:
            b_band_used = False
        ...
        # add in band resolutions
        if b_band_used:
            for cathode_setting in self.l_cathode_settings_in_use:
                # remember that single copy arrays are last element
                # so use 3rd and 2nd to last
                d_field_degree_pars[cathode_setting][-4]['band_res_s1'] = a_parameters[-3]
                d_field_degree_pars[cathode_setting][-4]['band_res_s2'] = a_parameters[-2]
            counter += 2
    ...
    def initial_positions_for_ensemble(self, a_free_parameters, num_walkers):
        ...
        if self.fit_type == 's':
            ...
            l_par_names = ['w_value', 'beta', 'kappa', 'eta', 'lambda', 'intrinsic_res_s1', 'intrinsic_res_s2', 'g1_value', 'spe_res_value', 'extraction_efficiency_value', 'gas_gain_mean_value', 'gas_gain_width_value', 'pf_eff_par0', 'pf_eff_par1', 's2_eff_par0', 's2_eff_par1', 'pf_stdev_par0', 'pf_stdev_par1', 'pf_stdev_par2', 'dpe_prob'] + l_field_free_pars + ['band_res_s1', 'band_res_s2']
        ...
        if not loaded_prev_sampler:
            ...
            if self.fit_type == 's' and self.num_dimensions == 21:
                ...
                if degree_setting == 2300:
                    ...
                    if cathode_setting == 1.054:
                        a_free_parameter_guesses = [6.72832478, 6.56628026, 0.76577538, 0.35899781, 0.1498054, 3.11178356]
                        # [ 7.56098549,  5.76996929,  0.07485624 ,  0.25600894, 0.38586383,  0.98883613] # 3000
                    
                    elif cathode_setting == 0.345:
                        a_free_parameter_guesses = [7.79099848, 6.65995579, 0.55237674, 0.29309462, 0.21676096, 2.68384366]
                        # [ 8.58012424,  6.4082583,  0.059447,  0.32507151, 0.28044492,  0.97627979] # 3500
                        
                    elif cathode_setting == 2.356:
                        a_free_parameter_guesses = [7.82229721, 6.51289495, 0.58805594, 0.23551629, 0.2363043, 2.60280798]
                        # [ 8.20653994,  7.45641684,  0.06750863 ,  0.21867223,  0.28004732, 0.92374844] # 3000
                        
                elif degree_setting == 3500:
                    if cathode_setting == 1.054:
                        a_free_parameter_guesses = [8.21616621, 5.80671977, 0.37694312, 0.18566468, 0.02616779, 1.84548649]
                        # [ 7.71422274,  6.03296573,  0.03810478,  0.05889366, 0.31801688,  0.91945319] # 3500
                        
                    elif cathode_setting == 0.345:
                        a_free_parameter_guesses = [8.0455475, 5.45732354, 0.22854824, 0.27415258, 0.05615733, 1.92447875]
                        # [ 7.76790283,  6.17368043,  0.21353381,  0.26405303, 0.05305485,  0.92298929] # 3500
                        
                    elif cathode_setting == 2.356:
                        a_free_parameter_guesses = [8.41909664, 5.97579379, 0.24121953, 0.17239215, 0.02838716, 1.71528222]
                        # [ 7.78806321,  6.27176934,  0.19965031 ,  0.17596064,  0.164257, 0.99944881] # 3500
                        
                elif degree_setting == 4500:
                    if cathode_setting == 1.054:
                        a_free_parameter_guesses = [8.56758353, 5.71379511, 0.29270284, 0.05429793, 0.0425484, 1.57764871]
                        # [ 7.78910186,  5.832333,  0.17891269,  0.22816425, 0.02462351,  0.99851682] # 4500
                    
                    elif cathode_setting == 0.345:
                        a_free_parameter_guesses = [8.21469736, 5.56229612, 0.34850862, 0.07296655, 0.34654592, 1.7858017]
                        # [ 8.82078054,  5.96911495,  0.1908735 ,  0.02383425,  0.48874606, 0.99311512] # 4500
                        
                    elif cathode_setting == 2.356:
                        a_free_parameter_guesses = [8.71534542, 5.97757567, 0.28413375, 0.05292223, 0.22320855, 1.56424734]
                        # [ 7.65321071,  5.96063235,  0.2200952 ,  0.0500525,  0.31784079, 0.98759496] # 4500
                    
                elif degree_setting == 5300:
                    if cathode_setting == 1.054:
                        a_free_parameter_guesses = [1.01115275e+01, 5.98088846e+00, 2.89235925e-01, 4.35593790e-02, 1.00096407e-02, 1.45109606e+00]
                        # [ 9.7874405,  6.12108864,  0.27462468,  0.01749649, 0.16826173,  0.99297773] # 5300
                        
                    elif cathode_setting == 0.345:
                        a_free_parameter_guesses = [10.49167185, 6.15234232, 0.22807023, 0.01969207, 0.0256287, 1.3875963]
                        # [ 9.82589491,  6.18360509,  0.20806021,  0.01750014,  0.13195983,  0.96117175] # 5300
                        
                    elif cathode_setting == 2.356:
                        a_free_parameter_guesses = [10.11526769, 6.11819988, 0.23884407, 0.04170792, 0.03723745, 1.44369635]
    ...
    def differential_evolution_minimizer_free_pars(self, a_bounds, maxiter=250, tol=0.05, popsize=15, polish=False):
        def neg_log_likelihood_diff_ev(a_guesses):
            if self.fit_type == 's':
                ...
                a_guesses = [13.7] + list(a_guesses[:6]) + [test.g1_value, test.spe_res_value, test.extraction_efficiency_value, test.gas_gain_value, test.gas_gain_width, test.l_means_pf_eff_pars[0], test.l_means_pf_eff_pars[1], test.l_means_s2_eff_pars[0], test.l_means_s2_eff_pars[1], test.l_means_pf_stdev_pars[0], test.l_means_pf_stdev_pars[1], test.l_means_pf_stdev_pars[2], 0.20] + list(a_guesses[-(len(self.l_cathode_settings_in_use)*2 + len(self.l_cathode_settings_in_use)*(len(self.l_degree_settings_in_use)*2-1) + 2):])
    ...
    def suppress_likelihood(self, iterations=200, a_free_par_guesses=None):
        ...
        if self.fit_type == 's' and a_free_par_guesses == None:
            ...
            # 3000, 345: [7.79099848, 6.65995579, 0.55237674, 0.29309462, 0.21676096, 2.68384366]
            # 3500, 345: [8.0455475, 5.45732354, 0.22854824, 0.27415258, 0.05615733, 1.92447875]
            # 4500, 345: [8.21469736, 5.56229612, 0.34850862, 0.07296655, 0.34654592, 1.7858017]
            # 5300, 345: [10.49167185, 6.15234232, 0.22807023, 0.01969207, 0.0256287, 1.3875963]
            # 3000, 1054: [6.72832478, 6.56628026, 0.76577538, 0.35899781, 0.1498054, 3.11178356]
            # 3500, 1054: [8.21616621, 5.80671977, 0.37694312, 0.18566468, 0.02616779, 1.84548649]
            # 4500, 1054: [8.56758353, 5.71379511, 0.29270284, 0.05429793, 0.0425484, 1.57764871]
            # 5300, 1054: [1.01115275e+01, 5.98088846e+00, 2.89235925e-01, 4.35593790e-02, 1.00096407e-02, 1.45109606e+00]
            # 3000, 2356: [7.82229721, 6.51289495, 0.58805594, 0.23551629, 0.2363043, 2.60280798]
            # 3500, 2356: [8.41909664, 5.97579379, 0.24121953, 0.17239215, 0.02838716, 1.71528222]
            # 4500, 2356: [8.71534542, 5.97757567, 0.28413375, 0.05292223, 0.22320855, 1.56424734]
            # 5300, 2356: [10.11526769, 6.11819988, 0.23884407, 0.04170792, 0.03723745, 1.44369635]
            ...
            if degree_setting == 2300 and cathode_setting == 1.054:
                ...
                l_free_pars = [6.72832478, 6.56628026, 0.76577538, 0.35899781, 0.1498054, 3.11178356]
            elif degree_setting == 3000 and cathode_setting == 0.345:
                l_free_pars = [7.79099848, 6.65995579, 0.55237674, 0.29309462, 0.21676096, 2.68384366]
            elif degree_setting == 3000 and cathode_setting == 2.356:
                l_free_pars = [7.82229721, 6.51289495, 0.58805594, 0.23551629, 0.2363043, 2.60280798]
            
            elif degree_setting == 3500 and cathode_setting == 1.054:
                l_free_pars = [8.21616621, 5.80671977, 0.37694312, 0.18566468, 0.02616779, 1.84548649]
            elif degree_setting == 3500 and cathode_setting == 0.345:
                l_free_pars = [8.0455475, 5.45732354, 0.22854824, 0.27415258, 0.05615733, 1.92447875]
            elif degree_setting == 3500 and cathode_setting == 2.356:
                l_free_pars = [8.41909664, 5.97579379, 0.24121953, 0.17239215, 0.02838716, 1.71528222]
            
            elif degree_setting == 4500 and cathode_setting == 1.054:
                l_free_pars = [8.56758353, 5.71379511, 0.29270284, 0.05429793, 0.0425484, 1.57764871]
            elif degree_setting == 4500 and cathode_setting == 0.345:
                l_free_pars = [8.21469736, 5.56229612, 0.34850862, 0.07296655, 0.34654592, 1.7858017]
            elif degree_setting == 4500 and cathode_setting == 2.356:
                l_free_pars = [8.71534542, 5.97757567, 0.28413375, 0.05292223, 0.22320855, 1.56424734]
            
            elif degree_setting == 5300 and cathode_setting == 1.054:
                l_free_pars = [1.01115275e+01, 5.98088846e+00, 2.89235925e-01, 4.35593790e-02, 1.00096407e-02, 1.45109606e+00]
            elif degree_setting == 5300 and cathode_setting == 0.345:
                l_free_pars = [10.49167185, 6.15234232, 0.22807023, 0.01969207, 0.0256287, 1.3875963]
            elif degree_setting == 5300 and cathode_setting == 2.356:
                l_free_pars = [10.11526769, 6.11819988, 0.23884407, 0.04170792, 0.03723745, 1.44369635]
        ...
        elif self.fit_type == 'mlti' and a_free_par_guesses == None:
            # -7611.2 (11.03)
            #[2.54939588e+03, 1.57619946e-01, 5.41196862e+00, 1.75879471e+00, 1.99031263e-01, 2.53147460e-01, 6.53197967e-03, 9.49893129e-01, 2.54523355e+00, 4.57287900e-01, 1.14810646e+00, 2.50333894e-01, 2.75434024e+00, 2.39239144e-02, 2.07915289e+00, 2.14821200e-01, 1.58167711e+00, 5.27447143e-03, 1.31299744e+00, 2.68573145e+00, 3.76481584e-01, 1.72374777e+00, 2.06270516e-01, 1.66246384e+00, 2.81156302e-01, 2.10244886e+00, 2.98699789e-01, 1.49194186e+00, 6.24836144e-03, 1.06208467e+00, 2.18957740e+00, 1.88771843e-01, 2.24797129e+00, 1.11121062e-01, 1.64893837e+00, 7.57261406e-02, 2.57242159e+00, 4.70625362e-01, 2.11841929e+00, 2.10727420e-01, 1.59032232e-01]
            a_free_par_guesses = [13.7] + [2.54939588e+03, 1.57619946e-01, 5.41196862e+00, 1.75879471e+00, 1.99031263e-01, 2.53147460e-01, test.g1_value, test.spe_res_value, test.extraction_efficiency_value, test.gas_gain_value, test.gas_gain_width, test.l_means_pf_eff_pars[0], test.l_means_pf_eff_pars[1], test.l_means_s2_eff_pars[0], test.l_means_s2_eff_pars[1], test.l_means_pf_stdev_pars[0], test.l_means_pf_stdev_pars[1], test.l_means_pf_stdev_pars[2], 0.20]
            a_free_par_guesses += [6.53197967e-03, 9.49893129e-01, 2.54523355e+00, 4.57287900e-01, 1.14810646e+00, 2.50333894e-01, 2.75434024e+00, 2.39239144e-02, 2.07915289e+00, 2.14821200e-01, 1.58167711e+00, 5.27447143e-03, 1.31299744e+00, 2.68573145e+00, 3.76481584e-01, 1.72374777e+00, 2.06270516e-01, 1.66246384e+00, 2.81156302e-01, 2.10244886e+00, 2.98699789e-01, 1.49194186e+00, 6.24836144e-03, 1.06208467e+00, 2.18957740e+00, 1.88771843e-01, 2.24797129e+00, 1.11121062e-01, 1.64893837e+00, 7.57261406e-02, 2.57242159e+00, 4.70625362e-01, 2.11841929e+00, 2.10727420e-01, 1.59032232e-01]

            # lower by 150/169
            #a_free_par_guesses = [13.903024507496928, 4731.6597384740508, 0.19389348198269879, 2.9283677690140615, 0.78954258218639106, 0.47014252491775677, 0.42733264777839242, 0.11461711195252081, 0.6721008524977381, 0.8671116688415087, 23.782246523571501, 8.5104065630365593, 3.128618380929288, 0.74908340068378088, 41.003608246202923, 329.20356365150258, 0.014646859822302041, 0.52203446900918993, 7.032053990367328, 0.18988738816920375, 0.0078292442999086994, 1.1191048695284587, 1.9460673060345721, 0.32067189820961484, 2.2250136234220901, 0.17196606331193137, 1.7172577536040876, 0.15036099213142162, 1.3508234775492574, 0.099667136376801815, 1.5412606492493348, 0.0083559444305281901, 1.0764057918429748, 1.966999087780134, 0.58054781241829012, 1.881764427808799, 0.2323939545844548, 1.5669454337411777, 0.49474573748359124, 1.7912233370630695, 0.25637394185664925, 1.6120100073962367, 0.0083422280843300431, 1.0793322839700137, 2.0503017411853288, 0.35561226760542897, 2.4813907361675911, 0.16943116108287345, 1.7465739716446083, 0.31260507966007289, 1.6277768176474816, 0.39602263071006283, 1.5859282551102616, 0.21095669102394771, 0.091357501976137956]
...
if __name__ == '__main__':
    ...
    test = fit_nr('s', d_coincidence_data, num_mc_events=2e4, l_gpus=[0], num_loops=4)
    ...
    # 4500, 2356: [8.71534542, 5.97757567, 0.28413375, 0.05292223, 0.22320855, 1.56424734]
    # 5300, 2356: [10.11526769, 6.11819988, 0.23884407, 0.04170792, 0.03723745, 1.44369635]
    l_test_parameters_single_energy = [7.73, 6.15, 0.01, 0.24, test.g1_value, test.spe_res_value, test.extraction_efficiency_value, test.gas_gain_value, test.gas_gain_width, test.l_means_pf_eff_pars[0], test.l_means_pf_eff_pars[1], test.l_means_s2_eff_pars[0], test.l_means_s2_eff_pars[1], test.l_means_pf_stdev_pars[0], test.l_means_pf_stdev_pars[1], test.l_means_pf_stdev_pars[2], 0, 0, 0, 0.21, 2.41]
    test.gpu_pool.map(test.wrapper_ln_likelihood_full_matching_single_energy, [l_test_parameters_single_energy])

    #a_free_par_bounds = [(3., 15.), (3.5, 15.), (0.01, 1.0), (0.01, 0.9), (0., 1.), (0.5, 3.5)]
    ...
    test = fit_nr('mlti', d_coincidence_data, num_mc_events=2e4, l_gpus=[0], num_loops=4)
    
    print 'Must change CUDA code to reflect change in smearing due to PF (multiply by MCS1
    
    #[2.54939588e+03, 1.57619946e-01, 5.41196862e+00, 1.75879471e+00, 1.99031263e-01, 2.53147460e-01, 6.53197967e-03, 9.49893129e-01, 2.54523355e+00, 4.57287900e-01, 1.14810646e+00, 2.50333894e-01, 2.75434024e+00, 2.39239144e-02, 2.07915289e+00, 2.14821200e-01, 1.58167711e+00, 5.27447143e-03, 1.31299744e+00, 2.68573145e+00, 3.76481584e-01, 1.72374777e+00, 2.06270516e-01, 1.66246384e+00, 2.81156302e-01, 2.10244886e+00, 2.98699789e-01, 1.49194186e+00, 6.24836144e-03, 1.06208467e+00, 2.18957740e+00, 1.88771843e-01, 2.24797129e+00, 1.11121062e-01, 1.64893837e+00, 7.57261406e-02, 2.57242159e+00, 4.70625362e-01, 2.11841929e+00, 2.10727420e-01, 1.59032232e-01]
    a_free_par_guesses = [13.7] + [2.54939588e+03, 1.57619946e-01, 5.41196862e+00, 1.75879471e+00, 1.99031263e-01, 2.53147460e-01, test.g1_value, test.spe_res_value, test.extraction_efficiency_value, test.gas_gain_value, test.gas_gain_width, test.l_means_pf_eff_pars[0], test.l_means_pf_eff_pars[1], test.l_means_s2_eff_pars[0], test.l_means_s2_eff_pars[1], test.l_means_pf_stdev_pars[0], test.l_means_pf_stdev_pars[1], test.l_means_pf_stdev_pars[2], 0.20]
    a_free_par_guesses += [6.53197967e-03, 9.49893129e-01, 2.54523355e+00, 4.57287900e-01, 1.14810646e+00, 2.50333894e-01, 2.75434024e+00, 2.39239144e-02, 2.07915289e+00, 2.14821200e-01, 1.58167711e+00, 5.27447143e-03, 1.31299744e+00, 2.68573145e+00, 3.76481584e-01, 1.72374777e+00, 2.06270516e-01, 1.66246384e+00, 2.81156302e-01, 2.10244886e+00, 2.98699789e-01, 1.49194186e+00, 6.24836144e-03, 1.06208467e+00, 2.18957740e+00, 1.88771843e-01, 2.24797129e+00, 1.11121062e-01, 1.64893837e+00, 7.57261406e-02, 2.57242159e+00, 4.70625362e-01, 2.11841929e+00, 2.10727420e-01, 1.59032232e-01]
    #a_free_par_guesses += [8.34029109e-03, 1.01880182e+00, 2.29015702e+00, 0.1, 2.06198166e+00, 0.2, 1.88959844e+00, 0.3, 9.66014598e-01, 0.4, 1.41115674e+00, 7.88334373e-03, 1.03047572e+00, 2.13392410e+00, 0.1, 2.05459463e+00, 0.2, 1.42532028e+00, 0.3, 1.23188392e+00, 0.4, 1.83180131e+00, 7.19667570e-03, 1.10080082e+00, 2.25763857e+00, 0.1, 1.73475899e+00, 0.2, 1.78668045e+00, 0.3, 2.77692256e+00, 0.4, 1.62673949e+00, 2.05864442e-01, 1.58916891e-01]
    #a_free_par_guesses = [13.561045826742996, 1279.8491940310264, 0.19536034576568373, 4.8628586757287708, 0.93379087598492061, 0.15351192410519104, 0.31274502263296161, 0.11055913408183073, 0.64452150035952827, 0.89717029650636249, 23.589874753073023, 10.428258162530687, 3.1813808154446037, 0.73113449357247384, 4.2977980954246142, 490.42401045417705, 0.014661361958772162, 0.53290923857515149, 6.9374188996158033, 0.2337754745401448, 0.0077985477170216394, 1.2661583936129035, 1.916260983281781, 0.45426587657814765, 2.0728278646228122, 0.53875094546541047, 2.3236855125504747, 0.72647474217651609, 2.5735734600259201, 0.31959524912346482, 1.8232113876582821, 0.0087264858273542068, 1.1769851205219659, 2.0894845952734751, 0.65805132588741966, 2.6464226505623105, 0.43838775764230714, 2.6748944878481127, 0.64120025365439004, 2.4960230137227972, 0.32605797451623514, 1.7153293696063017, 0.0089712056941624579, 1.0631394511756345, 1.991805835670617, 0.2759322932380206, 2.069993260429321, 0.26066501932013803, 1.9681751953178277, 0.46360023028824365, 2.2589956807662563, 0.4069004726902013, 1.8536664384004773, 0.27314815397245606, 0.059541968408341577]
    ...
    #a_free_par_bounds = [(100, 4000), (0.1, 0.2), (1.5, 10), (0.8, 2), (0, 0.3), (0, 0.3)] + [(0.001, 0.05), (0.9, 1.5), (0.8, 3.), (0, 0.5), (0.8, 3.), (0, 0.5), (0.8, 3.), (0, 0.5), (0.8, 3.), (0, 0.5), (0.8, 3.)] + [(0.001, 0.05), (0.9, 1.5), (0.8, 3.), (0, 0.5), (0.8, 3.), (0, 0.5), (0.8, 3.), (0, 0.5), (0.8, 3.), (0, 0.5), (0.8, 3.)] + [(0.001, 0.05), (0.9, 1.5), (0.8, 3.), (0, 0.5), (0.8, 3.), (0, 0.5), (0.8, 3.), (0, 0.5), (0.8, 3.), (0, 0.5), (0.8, 3.)] + [(0, 0.3), (0, 0.3)]
    #test.differential_evolution_minimizer_free_pars(a_free_par_bounds, maxiter=150, popsize=5, tol=0.1)
    
    
    test.suppress_likelihood()
    #test.run_mcmc(num_steps=20, num_walkers=512)
    """
