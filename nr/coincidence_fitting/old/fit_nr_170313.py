...
class fit_nr(object):
    def __init__(self, fit_type, d_coincidence_data, d_band_data=None, free_exciton_to_ion_ratio=False, num_mc_events=5e6, name_notes=None, l_gpus=[0], num_loops=1):
        ...
        for cathode_setting in self.l_cathode_settings_in_use:
            ...
            for degree_setting in self.l_degree_settings_in_use:
                ...
                self.d_coincidence_data_information[cathode_setting][degree_setting]['s1_hist'] = Hist(*self.l_s1_settings, name='hS1_%.3f_kV' % (cathode_setting))
                ...
                self.d_coincidence_data_information[cathode_setting][degree_setting]['s2_hist'] = Hist(*self.l_s2_settings, name='hS2_%.3f_kV' % (cathode_setting))
                ...
                self.d_coincidence_data_information[cathode_setting][degree_setting]['log_s2_s1_hist'] = Hist2D(*(self.l_s1_settings+self.l_log_settings), name='hLogS2S1_%.3f_kV' % (cathode_setting))
        ...
        if fit_type == 's':
            ...
            if fit_type == 'ml':
                ...
                self.num_dimensions = 17 + len(self.l_cathode_settings_in_use)*len(self.l_degree_settings_in_use)*2 + len(self.l_cathode_settings_in_use)*(1+1) # 17 + scales + bkg + recombination + exciton-to-ion ratio
    ...
    # band matching
    def ln_likelihood_full_matching_band(self, w_value, kappa, intrinsic_res_s1, intrinsic_res_s2, g1_value, spe_res_value, extraction_efficiency_value, gas_gain_mean_value, gas_gain_width_value, pf_eff_par0, pf_eff_par1, s2_eff_par0, s2_eff_par1, pf_stdev_par0, pf_stdev_par1, pf_stdev_par2, dpe_prob, d_field_degree_pars, d_gpu_local_info, draw_fit=False):
        ...
        for cathode_setting in self.l_cathode_settings_in_use:
            for degree_setting in self.l_degree_settings_in_use:
                # recombination prior
                prior_ln_likelihood += self.get_prior_log_likelihood_gompertz_pars([d_field_degree_pars[cathode_setting][degree_setting]['gom_par0'], d_field_degree_pars[cathode_setting][degree_setting]['gom_par1'], d_field_degree_pars[cathode_setting][degree_setting]['gom_par2'], d_field_degree_pars[cathode_setting][degree_setting]['gom_par3']])
        ...
        for cathode_setting in self.l_cathode_settings_in_use:
            for degree_setting in self.l_degree_settings_in_use:
                ...
                gom_par0 = np.asarray(d_field_degree_pars[cathode_setting][degree_setting]['gom_par0'], dtype=np.float32)
                gom_par1 = np.asarray(d_field_degree_pars[cathode_setting][degree_setting]['gom_par1'], dtype=np.float32)
                gom_par2 = np.asarray(d_field_degree_pars[cathode_setting][degree_setting]['gom_par2'], dtype=np.float32)
                gom_par3 = np.asarray(d_field_degree_pars[cathode_setting][degree_setting]['gom_par3'], dtype=np.float32)
    ...
    def wrapper_ln_likelihood_full_matching_band(self, a_parameters, kwargs={}):
        ...
        d_field_degree_pars = {}
        counter = 0
        num_field_degree_pars = len(self.l_cathode_settings_in_use)*(6)
        ...
        for cathode_setting in self.l_cathode_settings_in_use:
            ...
            for degree_setting in self.l_degree_settings_in_use:
                ...
                # recombination
                d_field_degree_pars[cathode_setting][degree_setting]['gom_par0'] = a_parameters[-(num_field_degree_pars+1)+counter+0]
                d_field_degree_pars[cathode_setting][degree_setting]['gom_par1'] = a_parameters[-(num_field_degree_pars+1)+counter+1]
                d_field_degree_pars[cathode_setting][degree_setting]['gom_par2'] = a_parameters[-(num_field_degree_pars+1)+counter+2]
                d_field_degree_pars[cathode_setting][degree_setting]['gom_par3'] = a_parameters[-(num_field_degree_pars+1)+counter+3]
                
                # exciton to ion ratio
                d_field_degree_pars[cathode_setting][degree_setting]['exciton_to_ion_ratio'] = a_parameters[-(num_field_degree_pars+1)+counter+4]
                
                # scaling par
                d_field_degree_pars[cathode_setting][degree_setting]['scale'] = a_parameters[-(num_field_degree_pars+1)+counter+5]
    
                # move counter
                counter += 6
        
        
        return self.ln_likelihood_full_matching_band(*(list(a_parameters[:-(num_field_degree_pars+1)]) + [d_field_degree_pars, a_parameters[-1]]), **kwargs)
    ...
    def ln_likelihood_full_matching_multiple_energies_lindhard_model(self, w_value, kappa, intrinsic_res_s1, intrinsic_res_s2, g1_value, spe_res_value, extraction_efficiency_value, gas_gain_mean_value, gas_gain_width_value, pf_eff_par0, pf_eff_par1, s2_eff_par0, s2_eff_par1, pf_stdev_par0, pf_stdev_par1, pf_stdev_par2, dpe_prob, d_field_degree_pars, d_gpu_local_info, draw_fit=False):
        ...
        for cathode_setting in self.l_cathode_settings_in_use:
            for degree_setting in self.l_degree_settings_in_use:
                ...
                #start_time_mc = time.time()
                #f_temp, ax_temp = plt.subplots(1)
                #print self.d_energy_arrays[degree_setting]
                #ax_temp.hist(self.d_energy_arrays[degree_setting]['a_energy'])
                #f_temp.savefig('./help_%d_deg.png' % (degree_setting))
            
                tArgs = (d_gpu_local_info['rng_states'], drv.In(num_trials), drv.In(mean_field), d_gpu_local_info['d_gpu_energy'][degree_setting], d_gpu_local_info['gpu_energy_bkg'], drv.In(w_value), drv.In(exciton_to_ion_ratio), drv.In(gom_par0), drv.In(gom_par1), drv.In(gom_par2), drv.In(gom_par3), drv.In(kappa), drv.In(g1_value), drv.In(extraction_efficiency), drv.In(gas_gain_value), drv.In(gas_gain_width), drv.In(spe_res), drv.In(intrinsic_res_s1), drv.In(intrinsic_res_s2), drv.In(a_pf_stdev), drv.In(pf_eff_par0), drv.In(pf_eff_par1), drv.In(s2_eff_par0), drv.In(s2_eff_par1), drv.In(dpe_prob), drv.In(prob_bkg), drv.In(num_bins_s1), d_gpu_local_info['gpu_bin_edges_s1'], drv.In(num_bins_log), d_gpu_local_info['gpu_bin_edges_log'], drv.InOut(a_hist_2d), drv.In(num_loops))
    ...
    def wrapper_ln_likelihood_full_matching_multiple_energies_lindhard_model(self, a_parameters, kwargs={}):
        ...
        d_field_degree_pars = {}
        counter = 0
        num_field_degree_pars = len(self.l_cathode_settings_in_use)*(5) + len(self.l_degree_settings_in_use)*len(self.l_cathode_settings_in_use)*2
        ...
        for cathode_setting in self.l_cathode_settings_in_use:
            ...
            for degree_setting in self.l_degree_settings_in_use:
                ...
                # scaling par
                d_field_degree_pars[cathode_setting][degree_setting]['prob_bkg'] = a_parameters[-(num_field_degree_pars+1)+counter+5+count_degree_pars]
                count_degree_pars += 1
                d_field_degree_pars[cathode_setting][degree_setting]['scale'] = a_parameters[-(num_field_degree_pars+1)+counter+5+count_degree_pars]
                count_degree_pars += 1
                
                
    
            # move counter
            counter += 5 + count_degree_pars
        
        
        return self.ln_likelihood_full_matching_multiple_energies_lindhard_model(*(list(a_parameters[:-(num_field_degree_pars+1)]) + [d_field_degree_pars, a_parameters[-1]]), **kwargs)
    ...
    def ln_likelihood_full_matching_multiple_energies_lindhard_model_with_ti_recombination(self, w_value, kappa, intrinsic_res_s1, intrinsic_res_s2, g1_value, spe_res_value, extraction_efficiency_value, gas_gain_mean_value, gas_gain_width_value, pf_eff_par0, pf_eff_par1, s2_eff_par0, s2_eff_par1, pf_stdev_par0, pf_stdev_par1, pf_stdev_par2, dpe_prob, d_field_degree_pars, d_gpu_local_info, draw_fit=False):
        ...
        for cathode_setting in self.l_cathode_settings_in_use:
            for degree_setting in self.l_degree_settings_in_use:
                ...
                tArgs = (d_gpu_local_info['rng_states'], drv.In(num_trials), drv.In(mean_field), d_gpu_local_info['d_gpu_energy'][degree_setting], d_gpu_local_info['gpu_energy_bkg'], drv.In(w_value), drv.In(exciton_to_ion_ratio), drv.In(ti_par), drv.In(kappa), drv.In(g1_value), drv.In(extraction_efficiency), drv.In(gas_gain_value), drv.In(gas_gain_width), drv.In(spe_res), drv.In(intrinsic_res_s1), drv.In(intrinsic_res_s2), drv.In(a_pf_stdev), drv.In(pf_eff_par0), drv.In(pf_eff_par1), drv.In(s2_eff_par0), drv.In(s2_eff_par1), drv.In(dpe_prob), drv.In(prob_bkg), drv.In(num_bins_s1), d_gpu_local_info['gpu_bin_edges_s1'], drv.In(num_bins_log), d_gpu_local_info['gpu_bin_edges_log'], drv.InOut(a_hist_2d), drv.In(num_loops))
    ...
    def ln_likelihood_test(self, w_value, alpha, beta, gom_par0, gom_par1, gom_par2, gom_par3, kappa, intrinsic_res_s1, intrinsic_res_s2, g1_value, spe_res_value, extraction_efficiency_value, gas_gain_mean_value, gas_gain_width_value, pf_eff_par0, pf_eff_par1, s2_eff_par0, s2_eff_par1, pf_stdev_par0, pf_stdev_par1, pf_stdev_par2, dpe_prob, d_scale_pars, d_gpu_local_info, draw_fit=False):
        ...
        for cathode_setting in self.l_cathode_settings_in_use:
            ...
            # with beta
            tArgs = (d_gpu_local_info['rng_states'], drv.In(num_trials), drv.In(mean_field), d_gpu_local_info['d_gpu_energy'][self.l_degree_settings_in_use[0]], drv.In(w_value), drv.In(alpha), drv.In(beta), drv.In(gom_par0), drv.In(gom_par1), drv.In(gom_par2), drv.In(gom_par3), drv.In(kappa), drv.In(g1_value), drv.In(extraction_efficiency), drv.In(gas_gain_value), drv.In(gas_gain_width), drv.In(spe_res), drv.In(intrinsic_res_s1), drv.In(intrinsic_res_s2), drv.In(a_pf_stdev), drv.In(pf_eff_par0), drv.In(pf_eff_par1), drv.In(s2_eff_par0), drv.In(s2_eff_par1), drv.In(dpe_prob), drv.In(num_bins_s1), d_gpu_local_info['gpu_bin_edges_s1'], drv.In(num_bins_log), d_gpu_local_info['gpu_bin_edges_log'], drv.InOut(a_hist_2d), drv.In(num_loops))
    ...
    def initial_positions_for_ensemble(self, a_free_parameters, num_walkers):
        ...
        for par_name in l_par_names:
            ...
            if par_name == 'a_py':
                ...
                if self.fit_type == 'bl' or self.fit_type[:2] == 'ml':
    ...
    def run_mcmc(self, num_steps, num_walkers, num_threads=1, thin=1):
        ...
        if not loaded_prev_sampler:
            ...
            if self.fit_type == 's' and self.num_dimensions == 21:
                ...
                a_free_parameter_guesses = [ 0.15497802,  0.11933846,  0.07725158,  0.00388023,  1.1511109 , 0.27293066,  0.91234272,  0.48473627,  0.95649465,  0.29930879, 0.84773711,  0.31636529,  0.86608489,  0.00155603,  1.36607393, 0.09729951,  0.96580884,  0.20707715,  0.99629186,  0.41131316, 0.91969589,  0.37868557,  0.92585252,  0.00453509,  1.09051919, 0.2408106 ,  0.75759365,  0.10566743,  0.87424659,  0.41268344, 0.86850127,  0.35370299,  0.96914916]
    ...
    def differential_evolution_minimizer_free_pars(self, a_bounds, maxiter=250, tol=0.05, popsize=15, polish=False):
        def neg_log_likelihood_diff_ev(a_guesses):
            if self.fit_type == 's':
                ...
                a_guesses = [13.7] + list(a_guesses[:3]) + [test.g1_value, test.spe_res_value, test.extraction_efficiency_value, test.gas_gain_value, test.gas_gain_width, test.l_means_pf_eff_pars[0], test.l_means_pf_eff_pars[1], test.l_means_s2_eff_pars[0], test.l_means_s2_eff_pars[1], test.l_means_pf_stdev_pars[0], test.l_means_pf_stdev_pars[1], test.l_means_pf_stdev_pars[2], 0.20] + list(a_guesses[-(len(self.l_cathode_settings_in_use)*2 + len(self.l_cathode_settings_in_use)*len(self.l_degree_settings_in_use)*2):])
    ...
    def suppress_likelihood(self, iterations=200, a_free_par_guesses=None):
        ...
        if self.fit_type == 's' and a_free_par_guesses == None:
            ...
            # suppress-likelihood: -956.7
            #[ 0.15497802,  0.11933846,  0.07725158,  0.00388023,  1.1511109 , 0.27293066,  0.91234272,  0.48473627,  0.95649465,  0.29930879, 0.84773711,  0.31636529,  0.86608489,  0.00155603,  1.36607393, 0.09729951,  0.96580884,  0.20707715,  0.99629186,  0.41131316, 0.91969589,  0.37868557,  0.92585252,  0.00453509,  1.09051919, 0.2408106 ,  0.75759365,  0.10566743,  0.87424659,  0.41268344, 0.86850127,  0.35370299,  0.96914916]
            a_free_par_guesses = [13.7] + [0.15497802, 0.11933846, 0.07725158, test.g1_value, test.spe_res_value, test.extraction_efficiency_value, test.gas_gain_value, test.gas_gain_width, test.l_means_pf_eff_pars[0], test.l_means_pf_eff_pars[1], test.l_means_s2_eff_pars[0], test.l_means_s2_eff_pars[1], test.l_means_pf_stdev_pars[0], test.l_means_pf_stdev_pars[1], test.l_means_pf_stdev_pars[2], 0.20]
            a_free_par_guesses += [0.00388023,  1.1511109 , 0.27293066,  0.91234272,  0.48473627,  0.95649465,  0.29930879, 0.84773711,  0.31636529,  0.86608489,  0.00155603,  1.36607393, 0.09729951,  0.96580884,  0.20707715,  0.99629186,  0.41131316, 0.91969589,  0.37868557,  0.92585252,  0.00453509,  1.09051919, 0.2408106 ,  0.75759365,  0.10566743,  0.87424659,  0.41268344, 0.86850127,  0.35370299,  0.96914916]
            ...
            # Mod Gompertz with beta only!
            # 1.054 kV
            # 506.2
            #[1.08807875e+00, 1.19392864e+03, 4.07339449e-02, 2.39773973e-01, 2.87371330e+01, 4.16469149e-02, 9.47598348e-02, 4.14056518e-02, 1.87293478e-01, 2.04992577e-01, 7.56952256e-01]
            #a_free_par_guesses = [13.7, 1.08807875e+00, 1.19392864e+03, 4.07339449e-02, 2.39773973e-01, 2.87371330e+01, 4.16469149e-02, 9.47598348e-02, 4.14056518e-02, 1.87293478e-01, test.g1_value, test.spe_res_value, test.extraction_efficiency_value, test.gas_gain_value, test.gas_gain_width, test.l_means_pf_eff_pars[0], test.l_means_pf_eff_pars[1], test.l_means_s2_eff_pars[0], test.l_means_s2_eff_pars[1], test.l_means_pf_stdev_pars[0], test.l_means_pf_stdev_pars[1], test.l_means_pf_stdev_pars[2], 2.04992577e-01, 7.56952256e-01]
            
            # 2.356 kV
            # 515.0
            #[1.04089395e+00, 2.77948097e+03, 3.57936156e-02, 2.74867406e-01, 1.17093611e+01, 2.98233720e-02, 9.34189490e-02, 3.22633058e-02, 1.51778410e-01, 1.88818032e-01, 7.66600741e-01]
            #a_free_par_guesses = [13.7, 1.04089395e+00, 2.77948097e+03, 3.57936156e-02, 2.74867406e-01, 1.17093611e+01, 2.98233720e-02, 9.34189490e-02, 3.22633058e-02, 1.51778410e-01, test.g1_value, test.spe_res_value, test.extraction_efficiency_value, test.gas_gain_value, test.gas_gain_width, test.l_means_pf_eff_pars[0], test.l_means_pf_eff_pars[1], test.l_means_s2_eff_pars[0], test.l_means_s2_eff_pars[1], test.l_means_pf_stdev_pars[0], test.l_means_pf_stdev_pars[1], test.l_means_pf_stdev_pars[2], 1.88818032e-01, 7.66600741e-01]
            
            # 0.345 kV
            # 471.7
            #[8.67590762e-01, 2.59311628e+03, 1.62304653e-01, 1.86890755e-01, 2.49869178e+01, 3.23269246e-02, 1.02842689e-01, 3.75229439e-02, 1.39838726e-01, 2.22748018e-01, 7.66066464e-01]
            #a_free_par_guesses = [13.7, 8.67590762e-01, 2.59311628e+03, 1.62304653e-01, 1.86890755e-01, 2.49869178e+01, 3.23269246e-02, 1.02842689e-01, 3.75229439e-02, 1.39838726e-01, test.g1_value, test.spe_res_value, test.extraction_efficiency_value, test.gas_gain_value, test.gas_gain_width, test.l_means_pf_eff_pars[0], test.l_means_pf_eff_pars[1], test.l_means_s2_eff_pars[0], test.l_means_s2_eff_pars[1], test.l_means_pf_stdev_pars[0], test.l_means_pf_stdev_pars[1], test.l_means_pf_stdev_pars[2], 2.22748018e-01, 7.66066464e-01]
...
if __name__ == '__main__':
    ...
    test = fit_nr('s', d_coincidence_data, num_mc_events=2.5e4, num_gpus=1, num_loops=4)
    ...
    d_coincidence_data = {}
    d_coincidence_data['degree_settings'] = [3000, 3500, 4500, 5300]
    d_coincidence_data['cathode_settings'] = [0.345, 1.054, 2.356]
    
    test = fit_nr('ml', d_coincidence_data, num_mc_events=2e4, l_gpus=[0], num_loops=4)
    
    #a_free_par_guesses = [13.7] + [1.50570624e-01, 1.73586063e-01, 1.20784480e-01, test.g1_value, test.spe_res_value, test.extraction_efficiency_value, test.gas_gain_value, test.gas_gain_width, test.l_means_pf_eff_pars[0], test.l_means_pf_eff_pars[1], test.l_means_s2_eff_pars[0], test.l_means_s2_eff_pars[1], test.l_means_pf_stdev_pars[0], test.l_means_pf_stdev_pars[1], test.l_means_pf_stdev_pars[2], 0.20]
    #a_free_par_guesses += [8.28728080e-02, 2.50685523e-01, 3.47215755e+01, 2.97646444e-02, 1.17420176e+00, 3.29941916e-01, 8.60177855e-01, 1.37438771e-01, 8.81837797e-01, 2.82658988e-01, 7.59336611e-01, 3.93830317e-01, 9.75755515e-01, 4.09473409e-02, 1.83664545e-01, 3.48234408e+01, 3.78743406e-02, 1.29242185e+00, 1.18262763e-01, 7.30735165e-01, 2.50593149e-01, 7.81316055e-01, 3.63693355e-01, 9.32919132e-01, 3.13526304e-01, 8.52441265e-01, 5.72525297e-02, 2.44435244e-01, 3.42007248e+01, 2.61562638e-02, 1.27534975e+00, 1.26479405e-01, 8.01497954e-01, 2.27479810e-01, 8.70813328e-01, 3.94553333e-01, 9.02541013e-01, 3.48684210e-01, 9.43474342e-01]
    #test.gpu_pool.map(test.wrapper_ln_likelihood_full_matching_multiple_energies_lindhard_model, [a_free_par_guesses])
    
    #a_free_par_bounds = [(0.05, 0.2), (0, 0.3), (0, 0.3)] + [(0, 0.15), (0.1, 0.3), (10, 35), (0.01, 0.045), (0.9, 1.5), (0, 0.5), (0.6, 1), (0, 0.5), (0.6, 1), (0, 0.5), (0.6, 1), (0, 0.5), (0.6, 1)] + [(0, 0.15), (0.1, 0.3), (10, 35), (0.01, 0.045), (0.9, 1.5), (0, 0.5), (0.6, 1), (0, 0.5), (0.6, 1), (0, 0.5), (0.6, 1), (0, 0.5), (0.6, 1)] + [(0, 0.15), (0.1, 0.3), (10, 35), (0.01, 0.045), (0.9, 1.5), (0, 0.5), (0.6, 1), (0, 0.5), (0.6, 1), (0, 0.5), (0.6, 1), (0, 0.5), (0.6, 1)]
    #test.differential_evolution_minimizer_free_pars(a_free_par_bounds, maxiter=150, popsize=10, tol=0.05)

    #test.suppress_likelihood()
    #test.run_mcmc(num_steps=10, num_walkers=512)
    ...
    #a_free_par_bounds = [(0.05, 0.2), (0, 0.3), (0, 0.3)] + [(0.001, 0.05), (0.9, 1.5), (0, 0.5), (0.6, 1), (0, 0.5), (0.6, 1), (0, 0.5), (0.6, 1), (0, 0.5), (0.6, 1)] + [(0.001, 0.05), (0.9, 1.5), (0, 0.5), (0.6, 1), (0, 0.5), (0.6, 1), (0, 0.5), (0.6, 1), (0, 0.5), (0.6, 1)] + [(0.001, 0.05), (0.9, 1.5), (0, 0.5), (0.6, 1), (0, 0.5), (0.6, 1), (0, 0.5), (0.6, 1), (0, 0.5), (0.6, 1)]
    #test.differential_evolution_minimizer_free_pars(a_free_par_bounds, maxiter=150, popsize=10, tol=0.05)
