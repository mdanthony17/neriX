...
class LakeShore340(Instrument):
	...
	def read_range(self):
		...
		debug('reading range')
		self.serial_lock.acquire()
		self.serial_port.write('RANGE?\r')
		self.serial_port.flush()
		debug('after range write')
		now = time.time()
		reply = self.serial_port.read(2)
		self.serial_lock.release()
		debug('after range read')
		...
		#else:
		#	value = float(reply[:-1])
		#	process_value = ProcessValue(now, value, 'W')
		#	print process_value
	...
	def read_pid(self):
		...
		try:
			self.serial_lock.acquire()
			self.serial_port.write('PID? 3\r')
			self.serial_port.flush()
			now = time.time()
			reply = self.serial_port.read(19)
			self.serial_lock.release()
		...
		#	process_value = None
		#else:
		#	print '<%d, %s>' % (time.time(), str(reply[-1]))
	...
	def read_loop(self):
		# read pid
		self.serial_lock.acquire()
		self.serial_port.write('CSET? 1\r')
		self.serial_port.flush()
		now = time.time()
		reply = self.serial_port.read(8)
		self.serial_lock.release()

		debug('loop: module replied <%s>' % repr(reply))
		return True
...
if __name__ == '__main__':
	...
	if len(sys.argv) != 1:
		print >>sys.stderr, 'usage: python test_smacd.py'
		sys.exit(1)
