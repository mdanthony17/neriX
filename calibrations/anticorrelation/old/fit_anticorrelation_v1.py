...
def fit_anticorrelation(d_files, batch_mode=False):
	...
	def ln_prior_cs137(delta_cs137, low_edge_rolloff_cs137, high_edge_rolloff_cs137, exp_bkg_cs137, res_cs137, delta_estimate, exp_bkg_estimate):
		if delta_estimate*0.9 < delta_cs137 < delta_estimate*1.1 and 0.01 < low_edge_rolloff_cs137 < 50 and 0.01 < high_edge_rolloff_cs137 < 50 and exp_bkg_estimate*0.9 < exp_bkg_cs137 < exp_bkg_estimate*1.1 and 0.01 < res_cs137 < 0.05:
			return -res_cs137/.0001
		else:
			return -np.inf
	...
	def ln_prior_na22(delta_na22, low_edge_rolloff_na22, high_edge_rolloff_na22, exp_bkg_na22, res_na22, delta_estimate, exp_bkg_estimate):
		if delta_estimate*0.9 < delta_na22 < delta_estimate*1.1 and 0.01 < low_edge_rolloff_na22 < 50 and 0.01 < high_edge_rolloff_na22 < 50 and exp_bkg_estimate*0.9 < exp_bkg_na22 < exp_bkg_estimate*1.1 and 0.01 < res_na22 < 0.05:
			return -res_na22/.0001
		else:
			return -np.inf
	...
	def ln_posterior(a_parameters, d_s1_s2_arrays, d_source_functions):
		...
		if 'cs137' in d_source_functions:
			...
			cur_index += d_source_functions['cs137']['ln_prior'].__code__.co_argcount-2
			d_source_functions['cs137']['pars'] = a_parameters[prev_index:cur_index]
			prev_index = cur_index
		
		if 'na22' in d_source_functions:
			# need -1 since have to feed in the estimate
			cur_index += d_source_functions['na22']['ln_prior'].__code__.co_argcount-2
		...
		if len(d_source_functions) == 1:
			...
			#print 'wvalue'
			#print ln_prior_w_normal(w_value)
			tot_ln_likelihood += ln_prior_w_normal(w_value)
		...
		try:
			for source in d_source_functions:
				...
				#print 'after energy'
				tot_ln_likelihood += d_source_functions[source]['ln_prior'](*d_source_functions[source]['pars'], delta_estimate=d_energy_information[source]['func'].GetParameter(1), exp_bkg_estimate=d_energy_information[source]['func'].GetParameter(2))
				#print source
