...
def gas_gain_fitting(filename, num_electrons):
    ...
    par_names = ['mean_bot', 'width_bot'] + ['p%d_ampl' % (i + 1) for i in xrange(iNumElectrons)] + ['fd_center_tot', 'fd_shape_tot', 'fd_center_top', 'fd_shape_top', 'mean_top', 'width_top']
    ...
    s2TopBinning = [40, -10, 80]
    s2BotBinning = [40, -10, 80]
    ...
    current_analysis.add_cut('S2sPeak[] - S2sPeak[0] > 400')
    ...
    sThreshTop = '1./(1+exp(([%d] - y)/([%d])))' % (iNumElectrons + 4, iNumElectrons + 5)
    ...
    FitS2TopBottom = TF2('FitS2TopBottom', '(x + y < %f ? 1. : 0.)*(x > %f ? 1. : 0.)*(y > %f ? 1. : 0.)*(%s)*(%s)*(%s)' % (fS2Max, fS2BottomMin, fS2TopMin, sThreshTopBottom, sThreshTop, ' + '.join(aS2sComb)), 0, 100,
                         0, 100)
    ...
    FitS2TopBottom.SetParLimits(iNumElectrons + 4, -10, 10)
    FitS2TopBottom.SetParLimits(iNumElectrons + 5, 0, 5)
    #FitS2TopBottom.SetParLimits(iNumElectrons + 6, 0.1, 40)
    #FitS2TopBottom.SetParLimits(iNumElectrons + 7, 0, 10)
    FitS2TopBottom.SetParLimits(iNumElectrons + 6, 2, 35)
    FitS2TopBottom.SetParLimits(iNumElectrons + 7, 2, 20)
	
    for i in xrange(num_electrons):
		FitS2TopBottom.SetParLimits(i+3, 0, 10000)
    ...
    if fd_fixed:
        ...
        FitS2TopBottom.FixParameter(iNumElectrons + 4, fd_center_bot)
        FitS2TopBottom.FixParameter(iNumElectrons + 5, fd_shape_tot)
        ...
        FitS2TopBottom.SetParameter(iNumElectrons + 4, 1)
    ...
    fitResult = S2TopBottom.Fit('FitS2TopBottom', 'SMEL')
...
lFiles = ['nerix_140929_1115', 'nerix_141029_1110']
...
"""
lFiles = ['nerix_141203_1506', 'nerix_141208_1123', 'nerix_141210_1028','nerix_141217_1041', 'nerix_150106_1100', 'nerix_150106_1137', 'nerix_150106_1234', 'nerix_150106_1404', 'nerix_150114_1407', 'nerix_150128_1158',
