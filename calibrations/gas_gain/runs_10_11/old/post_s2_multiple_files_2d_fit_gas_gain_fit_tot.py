...
def gas_gain_fitting(filename, num_electrons):
    ...
    s2TopBinning = [80, 0, 80]
    s2BotBinning = [80, 0, 80]
    ...
    hS2Tot = Hist(1000, 0, 250, name='hS2Tot', title='hS2Tot')
    hS2Tot.SetMarkerSize(0)
    current_analysis.Draw('(%s[] + S2sTotTop[])' % (s2_parameter), hist=hS2Tot, selection=current_analysis.get_cuts())
    hS2Tot.Draw()
    c3.Update()
    ...
    FitS2TopBottom = TF2('FitS2TopBottom', '(x + y < %f ? 1. : 0.)*(x > %f && x < %f ? 1. : 0.)*(y > %f && y < %f ? 1. : 0.)*(%s)*(%s)*(%s)*(%s)' % (fS2Max, fS2BottomMin, s2BotBinning[2], fS2TopMin, s2TopBinning[2], sThreshTopBottom, sThreshTop, s_asym_ternary, ' + '.join(aS2Total)), 0, 100,
                         0, 100)
