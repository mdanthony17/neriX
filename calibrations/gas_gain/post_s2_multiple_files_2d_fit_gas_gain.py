...
def gas_gain_fitting(filename, num_electrons):
    ...
    FitS2TopBottom = TF2('FitS2TopBottom', '(x + y < %f ? 1. : 0.)*(x > %f ? 1. : 0.)*(y > %f ? 1. : 0.)*(%s)*(%s)*(%s)*(%s)' % (fS2Max, fS2BottomMin, fS2TopMin, sThreshTopBottom, sThreshTop, s_asym_ternary, ' + '.join(aS2Total)), 0, 100,
                         0, 100)
