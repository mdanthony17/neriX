...
class fit_gas_gain:
    def __init__(self, filename, radial_cut=(0, 0.85), tot_threshold_high=80):
        ...
        hard_coded_correction_factor = 1.48e6 / 8.68e5#1.48e6 / 1.18e4 * (530./800.)**11.41
        neriX_analysis.warning_message('Using hard coded correction: %.2e' % hard_coded_correction_factor)
        tot_threshold_high *= hard_coded_correction_factor
        ...
        self.bot_branch = '(S2sTotBottom[S2Order[0]]*%f)' % hard_coded_correction_factor#'ctS2sTotBottom[S2Order[0]]'
        self.top_branch = '(S2sTotTop[S2Order[0]]*%f)' % hard_coded_correction_factor
        ...
        self.l_bot_binning = [60, 0, 60*hard_coded_correction_factor]
        self.l_top_binning = [60, 0, 60*hard_coded_correction_factor]
        ...
        self.dict_filename = 'sampler_dictionary_thesis.p'
        self.acceptance_filename = 'acceptance_fraction.p'
        
        
        self.a_free_par_guesses = [13.5*hard_coded_correction_factor, 4.9*hard_coded_correction_factor, 15.5*hard_coded_correction_factor, 5.*hard_coded_correction_factor, 0., 5., 5., 5., 3., 0.36, 0.05]
    ...
    def draw_gas_gain_corner_plot(self, num_walkers, num_steps_to_include):
        ...
        fig.savefig(self.s_directory_save_name + self.s_base_save_name + '_corner_thesis.png')
    ...
    def draw_best_fit_on_hist(self, num_walkers, num_steps_to_include):
        ...
        l_levels = np.array([0.05, 0.1, 0.2, 0.3, 0.5, 0.9])*np.max(z)
    
        ax1.contour(x, y, z, cmap=plt.get_cmap('Greys'), levels=l_levels)
        #ax1.set_yscale('log', nonposx='clip')
        #ax1.set_title('Gas Gain - %s' % (self.filename))
        ax1.set_xlabel(r'$\mathrm{S2}_{\mathrm{top}}$ [PE]')
        ax1.set_ylabel(r'$\mathrm{S2}_{\mathrm{top}}$ [PE]')
    
        #plt.show()
        f1.savefig(self.s_directory_save_name + self.s_base_save_name + '_hist_thesis.png')
...
if __name__ == '__main__':
    ...
    test.run_gas_gain_mcmc(num_walkers=64, num_steps=100, threads=7)
