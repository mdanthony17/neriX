...
current_analysis.add_cut('(S2sCoin[] > 4)')
#current_analysis.add_cut('(S2sPeak[] > 6000 && S2sPeak[] < 12000)')
current_analysis.add_cut('(S2sPeak[] > (S2sPeak[0] + 100) && S2sPeak[] < (S2sPeak[0] + 2000))')
current_analysis.add_dt_cut()

current_analysis.add_cut('sqrt(pow(S2sPosFann[][0],2)+pow(S2sPosFann[][1],2)) < 0.7')

#current_analysis.add_cut('(S2sTotBottom[] > 15 && S2sTotBottom[] < 25)')
#current_analysis.get_T1().Scan('EventId:S2sTot[]:S2sTotTop[]:S2sTotBottom[]:S2sPeak[]', current_analysis.get_cuts())
...
#tree.Scan('EventId:ctgS2sTot', str(basic_cuts) + ' && s2stot[] > 15')


s2_texts = []
s2_funcs = []
s2_total_func_exp = '[2]*exp(-0.5*((x-[0])/[1])**2)'
...
s2_hist.Fit(s2_total_func, '0ME', '', *s2_fit_limits)
...
for i, s2_func in enumerate(s2_funcs):
	s2_func.SetParameters(s2_total_func.GetParameter(0), s2_total_func.GetParameter(1), s2_total_func.GetParameter(i+2))
...
s2_fit_text = root.TLatex(0.46, 0.76, '#mu_{S2, e-} #pm #sigma_{S2, e-} = %0.1f #pm %0.1f pe' % (s2_total_func.GetParameter(0), s2_total_func.GetParameter(1)))
s2_fit_text.SetNDC()

y_max = int(1.4*s2_hist.GetMaximum())

s2_threshold_text = root.TLatex(0.46, 0.70, 'S2_{0} = %0.1f pe,  #beta_{S2} = %0.1f pe' % (s2_total_func.GetParameter(max_nb_electrons+2), s2_total_func.GetParameter(max_nb_electrons+3)))
