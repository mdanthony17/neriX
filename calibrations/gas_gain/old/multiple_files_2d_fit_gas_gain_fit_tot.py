...
def gas_gain_fitting(filename, num_electrons):
    ...
    s2TopBinning = [40, 0, 80]
    s2BotBinning = [40, 0, 80]
    ...
    FitS2TopBottom = TF2('FitS2TopBottom', '(x + y < %f ? 1. : 0.)*(x > %f && x < %f ? 1. : 0.)*(y > %f && y < %f ? 1. : 0.)*(%s)*(%s)*(%s)' % (fS2Max, fS2BottomMin, s2BotBinning[2], fS2TopMin, s2TopBinning[2], sThreshTopBottom, sThreshTop, ' + '.join(aS2Total)), 0, 100,
                         0, 100)
