...
def gas_gain_fitting(filename, num_electrons):
    ...
    aColors = [4, 2, 8, 7, 5, 9] + [4, 2, 8, 7, 5, 9] + [4, 2, 8, 7, 5, 9]
    ...
    if current_analysis.get_cathode_setting() == 0.345:
        photoionization_windows_cathode = [1600, 1750]
    elif current_analysis.get_cathode_setting() == 1.054:
        photoionization_windows_cathode = [1450, 1600]
    elif current_analysis.get_cathode_setting() == 2.356:
        photoionization_windows_cathode = [1300, 1400]
    elif current_analysis.get_cathode_setting() == 5.500:
        photoionization_windows_cathode = [1150, 1310]
    ...
    # total S2 cut
    current_analysis.add_cut('%s[] + S2sTotTop[] < %f' % (s2_parameter, fS2Max))

    # S2 bottom low cut
    current_analysis.add_cut('%s[] > %f' % (s2_parameter, fS2BottomMin))

    # S2 top low cut
    current_analysis.add_cut('S2sTotTop[] > %f' % (fS2TopMin))

    # S2 bottom high cut
    current_analysis.add_cut('%s[] < %f' % (s2_parameter, s2BotBinning[2]))

    # S2 top high cut
    current_analysis.add_cut('S2sTotTop[] < %f' % (s2TopBinning[2]))
	
    # coincidence cut
    # current_analysis.add_cut('S2sCoin[S2Order[0]] > %i'%iCoinPMTs)


    # sample range after S2
    #current_analysis.add_cut('S2sPeak[] - S2sPeak[0] > 1500 && !(S2sPeak[] - S2sPeak[0] > %d && S2sPeak[] - S2sPeak[0] < %d)' % (photoionization_windows_cathode[0], photoionization_windows_cathode[1]))
    current_analysis.add_cut('(S2sPeak[] - S2sPeak[0] > %d && S2sPeak[] - S2sPeak[0] < %d)' % (photoionization_windows_cathode[0], photoionization_windows_cathode[1]))
    ...
    #current_analysis.add_cut('Alt$(S2sLeftEdge[S2Order[1]],100000) - S2sRightEdge[S2Order[%s]] > %i' % (s2_order, iSamplesBetweenS2))
    ...
    #current_analysis.add_cut('S2sTotTop[] > 200')
    #current_analysis.get_T1().Scan('EventId:S2sTotTop[]:ctS2sTotBottom[]:S2sPeak[]', current_analysis.get_cuts())
    ...
    c2 = Canvas()
    current_analysis.Draw('(%s[] - S2sTotTop[]) / (%s[] + S2sTotTop[])' % (s2_parameter, s2_parameter), hist=hS2Asym, selection=current_analysis.get_cuts())
    hS2Asym.Draw()
    c2.Update()


    fAsymGaus = TF1('fAsymGaus', 'gaus', -1, 1)
    fAsymGaus.SetParameters(100, 0., 0.3)
    hS2Asym.Fit('fAsymGaus', 'Q')

    #current_analysis.add_cut('((%s[] - S2sTotTop[]) / (%s[] + S2sTotTop[]) > %f) && ((%s[] - S2sTotTop[]) / (%s[] + S2sTotTop[]) < %f)' % (s2_parameter, s2_parameter, fAsymGaus.GetParameter(1) - asym_width*fAsymGaus.GetParameter(2), s2_parameter, s2_parameter, fAsymGaus.GetParameter(1) + asym_width*fAsymGaus.GetParameter(2)))
    ...
    current_analysis.Draw('%s[]:S2sTotTop[]' % s2_parameter, hist=S2TopBottom, selection=current_analysis.get_cuts())
    ...
    #print '(x + y < %f ? 1. : 0.)*(x > %f ? 1. : 0.)*(y > %f ? 1. : 0.)*(%s)*(%s)*(%s)*(%s)' % (fS2Max, fS2BottomMin, fS2TopMin, sThreshTopBottom, sThreshTop, s_asym_ternary, ' + '.join(aS2Total))

    # print '(%s)*(%s)*(%s)'%(sThreshBottom,sThreshTop,' + '.join(aS2sComb))
    S2TopBottom.GetXaxis().SetTitle('%s[]' % s2_parameter)
    ...
    if fd_fixed:
        ...
        FitS2TopBottom.SetParameter(iNumElectrons + 8, .3)
    ...
    c1.SaveAs(sPathToSaveOutput + '/gas_gain_' + sFileName + '_post_s2.png')
    c1.SaveAs(sPathToSaveOutput + '/gas_gain_' + sFileName + '_post_s2.C')
...
for file in lFiles:
	...
	print lLnlikelihoods
	
	countNonZero = 0
	for likelihood in lLnlikelihoods:
		if likelihood != 0:
			countNonZero += 1
