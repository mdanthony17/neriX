...
class fit_position_correction:
    ...
    def draw_pos_correction_plots(self, analysis_name, cathode_setting, num_walkers, num_steps_to_include):
        ...
        a_z_bins = np.linspace(-24, -3, 15)
        a_scaled_s1_bins = np.linspace(0.65, 1.4, 15)
        a_dt_bins = np.linspace(2, 14, 15)
        a_s2_bins = np.linspace(5.5e5, 8.5e5, 15)

        f1, (ax1) = plt.subplots(1)
        if analysis_name == 's1_z':
            #ax1.plot(self.d_info[cathode_setting]['df_tree']['z'], self.d_info[cathode_setting]['a_scaled_s1'], color='b', linestyle='', marker='.')
            ax1.hist2d(self.d_info[cathode_setting]['df_tree']['z'], self.d_info[cathode_setting]['a_scaled_s1'], bins=[a_z_bins, a_scaled_s1_bins], cmap='Blues')
            
            x_line = np.linspace(self.l_z_bounds[0], self.l_z_bounds[1], 50)
            y_line = a_medians[0] + a_medians[1]*x_line
            ax1.plot(x_line, y_line, color='magenta', linestyle='--', linewidth=2)
            
            #ax1.set_title('Scaled S1 vs. Z - %.3f kV' % (cathode_setting))
            ax1.set_xlabel('Z [mm]')
            ax1.set_ylabel('Scaled S1')
        ...
        if analysis_name == 's2_dt':
            ...
            ax1.hist2d(self.d_info[cathode_setting]['df_tree']['dt'], self.d_info[cathode_setting]['df_tree']['s2'], bins=[a_dt_bins, a_s2_bins], cmap='Blues')
            ...
            ax1.set_xlabel(r'dt [$\mu$s]')
            ax1.set_ylabel('S2 [PE]')

        f1.savefig(self.s_directory_save_name + self.s_base_save_name + '_%s_thesis.png' % (analysis_name))
