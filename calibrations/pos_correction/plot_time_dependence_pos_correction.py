...
l_cathode_settings = [0.345, 0.700, 1.054, 1.500, 2.356]
l_analysis_types = ['s1_z', 's2_dt']

for cathode_setting in l_cathode_settings:
    d_pos_correction[cathode_setting] = {}
    
    d_pos_correction[cathode_setting]['l_s1_z_intercept_median'] = []
    d_pos_correction[cathode_setting]['l_s1_z_intercept_mad'] = []
    
    d_pos_correction[cathode_setting]['l_s1_z_slope_median'] = []
    d_pos_correction[cathode_setting]['l_s1_z_slope_mad'] = []
...
d_pos_correction[1.5]['color'] = 'orange'
d_pos_correction[2.356]['color'] = 'red'
    ...
    for cathode_setting in l_cathode_settings:
        for analysis_type in l_analysis_types:
            ...
            if analysis_type == 's1_z':
                ...
                d_pos_correction[cathode_setting]['l_s1_z_intercept_median'].append(s1_z_intercept_median)
                d_pos_correction[cathode_setting]['l_s1_z_intercept_mad'].append(neriX_analysis.mad(a_sampler[:, index_s1_z_intercept]))
                ...
                d_pos_correction[cathode_setting]['l_s1_z_slope_median'].append(s1_z_slope_median)
                d_pos_correction[cathode_setting]['l_s1_z_slope_mad'].append(neriX_analysis.mad(a_sampler[:, index_s1_z_slope]))
...
for cathode_setting in l_cathode_settings:
    for i in xrange(len(d_pos_correction[cathode_setting]['l_e_lifetime_mad'])):
    
        l_intercept_means.append(d_pos_correction[cathode_setting]['l_s1_z_intercept_median'][i])
        l_intercept_stdevs.append(d_pos_correction[cathode_setting]['l_s1_z_intercept_mad'][i])

        l_slope_means.append(d_pos_correction[cathode_setting]['l_s1_z_slope_median'][i])
        l_slope_stdevs.append(d_pos_correction[cathode_setting]['l_s1_z_slope_mad'][i])
...
for cathode_setting in l_cathode_settings:
    ...
    #plot s1 z intercept
    ax1.errorbar(d_pos_correction[cathode_setting]['l_time'], d_pos_correction[cathode_setting]['l_s1_z_intercept_median'], yerr=d_pos_correction[cathode_setting]['l_s1_z_intercept_mad'], color=d_pos_correction[cathode_setting]['color'], marker='o', linestyle='')
    ...
    ax2.errorbar(d_pos_correction[cathode_setting]['l_time'], d_pos_correction[cathode_setting]['l_s1_z_slope_median'], yerr=d_pos_correction[cathode_setting]['l_s1_z_slope_mad'], color=d_pos_correction[cathode_setting]['color'], marker='o', linestyle='')
    ...
    ax3.errorbar(d_pos_correction[cathode_setting]['l_time'], d_pos_correction[cathode_setting]['l_e_lifetime_median'], yerr=[d_pos_correction[cathode_setting]['l_e_lifetime_minus_one_sigma'], d_pos_correction[cathode_setting]['l_e_lifetime_plus_one_sigma']], color=d_pos_correction[cathode_setting]['color'], marker='o', linestyle='')

    if cathode_setting in [0.345, 0.7, 1.054]:
        ax4.errorbar(d_pos_correction[cathode_setting]['l_time'], d_pos_correction[cathode_setting]['l_e_lifetime_median'], yerr=[d_pos_correction[cathode_setting]['l_e_lifetime_minus_one_sigma'], d_pos_correction[cathode_setting]['l_e_lifetime_plus_one_sigma']], color=d_pos_correction[cathode_setting]['color'], marker='o', linestyle='')
    


    # add labels for legends
    d_pos_correction[cathode_setting]['legend_entry'] = mpatches.Patch(color=d_pos_correction[cathode_setting]['color'], label=r'%.3f kV' % (cathode_setting))
    l_legend_handles.append(d_pos_correction[cathode_setting]['legend_entry'])
