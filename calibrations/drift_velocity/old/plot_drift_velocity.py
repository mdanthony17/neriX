...
parameterToDraw = '(S2sPeak[1]-S2sPeak[0])/100.'
...
run.add_cut('(S1sTot[0]>0 && S1sTot[0]<5000 && dt<20. && dt>0. && (S2sTot[1]/S2sTot[0])<0.02)')
run.set_event_list()
...
for i in xrange(0, 2):
	...
	leftFitEdge = lFits[i][0].GetParameter(1) - 2.*lFits[i][0].GetParameter(2)
	rightFitEdge = lFits[i][0].GetParameter(1) + 2.*lFits[i][0].GetParameter(2)
